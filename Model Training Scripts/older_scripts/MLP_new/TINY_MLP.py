...
def evaluate(model, loader, device, criterion, args, calibrator: Optional[ProbCalibrator] = None,
    ...
    # how many probs fall in [reliability_bin_min, reliability_bin_max]
    range_min = float(getattr(args, "reliability_bin_min", 0.0))
    range_max = float(getattr(args, "reliability_bin_max", 0.06))

    prob_inrange_h = np.zeros(num_horizons, dtype=np.float64)
    prob_total_h   = np.zeros(num_horizons, dtype=np.float64)
    ...
    for batch in iterator:
        ...
        for k_idx in range(K_eff):
            ...
            # NEW: count probabilities in the range
            in_range = (p_flat_t >= range_min) & (p_flat_t <= range_max)
    ...
    for idx, h in enumerate(args.horizons):
        ...
        prob_range_per_h[int(h)] = {"count": cnt, "total": tot, "fraction": frac}
...
def parse_args():
    ...
    p.add_argument(
    "--prob-max-pct", type=float, default=0.06,
    help="Max probability expressed in percent. 0.06 means 0.06% => 0.0006."
    )
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loader is not None:
            ...
            # --- NEW: how many probs fall in [reliability_bin_min, reliability_bin_max] ---
            pr = val_metrics.get("prob_range", {})
            if pr:
                print(
                    f"[Val] prob_range [{pr['min']:.6f}, {pr['max']:.6f}]: "
                    f"{pr['count']:.0f}/{pr['total']:.0f} ({100.0 * pr['fraction']:.3f}%)"
                )
                # Optional: per-horizon breakdown
                per = pr.get("per_horizon", {}) or {}
                for h, hh in per.items():
                    print(
                        f"  [Val prob_range h={h}] "
        ...
        if epoch == args.epochs and test_loader is not None:
            ...
            pr = test_metrics.get("prob_range", {})
            if pr:
                print(
                    f"[Test] prob_range [{pr['min']:.6f}, {pr['max']:.6f}]: "
                    f"{pr['count']:.0f}/{pr['total']:.0f} ({100.0 * pr['fraction']:.3f}%)"
                )
