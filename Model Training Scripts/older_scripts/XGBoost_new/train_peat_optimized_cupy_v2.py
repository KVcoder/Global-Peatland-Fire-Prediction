...
def _infer_expected_F_from_sample(sample_x: torch.Tensor) -> int:
    """Infer feature dimension F for tabular models from a single dataset sample."""
    if sample_x.dim() == 5:
        # (B,T,C,H,W) shouldn't happen for a single sample, but handle anyway
        _, T, C, _, _ = sample_x.shape
        return int(T * C)
    if sample_x.dim() == 4:
        # (T,C,H,W)
        T, C, _, _ = sample_x.shape
        return int(T * C)
    if sample_x.dim() == 3:
        # (F,H,W)
        F, _, _ = sample_x.shape
        return int(F)
    raise ValueError(f"Unexpected sample x shape for tabular inference: {tuple(sample_x.shape)}")


def _ensure_bfchw(x4: torch.Tensor, expected_F: Optional[int] = None) -> torch.Tensor:
    """Ensure x is (B,F,H,W). If channels-last (B,H,W,F), convert."""
    if x4.dim() != 4:
        raise ValueError(f"Expected 4D x (B,F,H,W) or (B,H,W,F), got {tuple(x4.shape)}")
    if expected_F is None:
        return x4

    if x4.shape[1] == expected_F:
        return x4

    # Common gotcha: some pipelines output channels-last (B,H,W,F)
    if x4.shape[-1] == expected_F:
        return x4.permute(0, 3, 1, 2).contiguous()

    raise ValueError(
        f"Tabular feature dim mismatch: got x shape {tuple(x4.shape)} but expected F={expected_F}. "
        "This usually means some samples are emitting a different feature set (e.g., coords-only) or a different layout."
    )


def batch_to_tabular_numpy(
    batch: Dict[str, Any],
    expected_F: Optional[int] = None,
    ...
    expected_F:
      If set, enforces a consistent feature dimension and also auto-fixes channels-last input.
    """
    x4 = _x_to_time_collapsed_channels(batch["x"])
    x4 = _ensure_bfchw(x4, expected_F=expected_F)
...
def collect_tabular_train_samples(train_ds, args) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    Key properties:
      - Enforces a *single* feature dimension F across all batches.
      - Optionally writes to memmap to keep RAM usage flat.
      - Uses a memory-friendly DataLoader config for collection (no pin_memory; small prefetch).
    """
    rng = np.random.default_rng(args.seed + 999)
    maxN = int(args.max_train_pixels)

    # Infer expected (F,K) from a single sample so we can preallocate and detect mismatches early.
    s0 = train_ds[0]
    expected_F = _infer_expected_F_from_sample(s0["x"])
    if not torch.is_tensor(s0["y"]) or s0["y"].dim() != 3:
        raise ValueError(f"Unexpected sample y shape: {tuple(getattr(s0['y'], 'shape', (None,)))}")
    expected_K = int(s0["y"].shape[0])

    # memmap (optional)
    use_mm = bool(getattr(args, "tabular_memmap", False))
    mm_dir = Path(getattr(args, "tabular_memmap_dir", None) or args.logdir)
    mm_dir.mkdir(parents=True, exist_ok=True)

    if use_mm:
        X_out = np.memmap(mm_dir / "tabular_X.dat", mode="w+", dtype=np.float32, shape=(maxN, expected_F))
        Y_out = np.memmap(mm_dir / "tabular_Y.dat", mode="w+", dtype=np.float32, shape=(maxN, expected_K))
        M_out = np.memmap(mm_dir / "tabular_M.dat", mode="w+", dtype=np.float32, shape=(maxN, expected_K))
    else:
        # Preallocate in RAM (still large for big maxN*F, but avoids list-of-arrays blowups).
        X_out = np.empty((maxN, expected_F), dtype=np.float32)
        Y_out = np.empty((maxN, expected_K), dtype=np.float32)
        M_out = np.empty((maxN, expected_K), dtype=np.float32)

    # Memory-friendly loader for collection (important with many workers/prefetch)
    pin = False
    kw: Dict[str, Any] = dict(
        ...
        pin_memory=pin,
        collate_fn=collate,
        persistent_workers=False,  # keep RAM lower during big collection
    )
    if args.workers > 0:
        kw["prefetch_factor"] = max(1, min(int(args.prefetch), 2))  # cap prefetch to avoid RAM explosion
        ctx = _choose_mp_context(args.mp_context)
        if ctx:
            kw["multiprocessing_context"] = ctx
    ...
    for batch in it:
        try:
            X, Y, M, _ = batch_to_tabular_numpy(batch, expected_F=expected_F)
        except Exception as e:  # noqa: BLE001
            bad_batches += 1
            if bad_batches <= 3:
                bx = batch.get("x", None)
                bx_shape = tuple(bx.shape) if torch.is_tensor(bx) else None
                meta0 = None
                try:
                    meta0 = (batch.get("meta", [{}])[0]) if isinstance(batch.get("meta", None), list) else None
                except Exception:
                    meta0 = None
                print(
                    f"[TabularCollect] Skipping batch due to feature-shape issue: {e} | x.shape={bx_shape} | meta0={meta0}"
                )
            continue

        if X.shape[1] != expected_F or Y.shape[1] != expected_K or M.shape[1] != expected_K:
            bad_batches += 1
            if bad_batches <= 3:
                print(
                    f"[TabularCollect] Skipping batch with inconsistent shapes: "
                    f"X{X.shape}, Y{Y.shape}, M{M.shape} (expected F={expected_F}, K={expected_K})"
                )
            continue
        ...
        n_take = min(int(args.pixels_per_batch), int(idx_all.size), int(remaining))
        idx = rng.choice(idx_all, size=n_take, replace=False)

        X_out[total:total + n_take] = X[idx].astype(np.float32, copy=False)
        Y_out[total:total + n_take] = Y[idx].astype(np.float32, copy=False)
        M_out[total:total + n_take] = M[idx].astype(np.float32, copy=False)

        total += n_take
        it.set_postfix({"tiles": total, "bad_batches": bad_batches})

    if total == 0:
        raise RuntimeError(
            "No valid tabular training tiles collected. "
            "If you see many 'Skipping batch' messages, your dataset is emitting inconsistent x feature shapes."
        )

    # Return only the filled portion. np.asarray on a memmap slice keeps it as an ndarray view (no copy).
    Xc = np.asarray(X_out[:total])
    Yc = np.asarray(Y_out[:total])
    Mc = np.asarray(M_out[:total])
    return Xc, Yc, Mc
