...
Backend support:
  - sklearn (CPU): Default, works everywhere
  - cuML (GPU): Use --use-gpu for 10-50x speedup (requires: pip install cuml-cu11 or cuml-cu12)
...
# ----------------------------------------------------------------------
# Tabular sampling for Random Forest
# ----------------------------------------------------------------------
...
def collect_tabular_samples(ds, args, split_name: str, monitor: Optional[RAMMonitor] = None) -> TabularSamples:
    """Sample valid pixels from patches to create manageable RF training matrices."""
    K = len(args.horizons)
...
def parse_args():
    ...
    p.add_argument("--use-gpu", action="store_true",
                   help="Use cuML GPU-accelerated Random Forest if available (requires cuML installed)")
    ...
    p.add_argument("--platt-on-gpu", action="store_true",
                   help="If set, optimize Platt a,b on CUDA (RF predictions are still CPU-based).")
...
def main():
    ...
    with contextlib.ExitStack() as stack:
        ...
        # Determine which Random Forest backend to use
        use_cuml = args.use_gpu and CUML_AVAILABLE
        if args.use_gpu and not CUML_AVAILABLE:
            print("[WARN] --use-gpu requested but cuML not available. Falling back to sklearn CPU.")
            print("[WARN] Install cuML: pip install cuml-cu11 (CUDA 11) or cuml-cu12 (CUDA 12)")

        RandomForestClassifier = cuMLRandomForestClassifier if use_cuml else SklearnRandomForestClassifier
        backend_name = "cuML (GPU)" if use_cuml else "sklearn (CPU)"

        print_diagnostic_header("Model")
        print_diagnostic_item("Random Forest per-horizon classifiers", "binary classification", indent=1)
        print_diagnostic_item("Backend", backend_name, indent=1)
        ...
        for k, h in enumerate(args.horizons):
            ...
            if use_cuml:
                ...
                params = dict(
                    ...
                    max_depth=int(args.rf_max_depth) if args.rf_max_depth > 0 else 16,  # cuML default
                    max_features=args.rf_max_features if args.rf_max_features != "sqrt" else "auto",  # cuML uses "auto"
                    ...
                    n_streams=4,  # cuML-specific: parallel CUDA streams
                )
                # cuML doesn't support max_samples, bootstrap, n_jobs (always uses GPU)
            else:
                # sklearn parameters (CPU)
                params = dict(
                    n_estimators=int(args.rf_n_estimators),
            ...
            sample_weight = None
            if args.rf_use_class_weight and not use_cuml:  # cuML doesn't support class_weight
                params["class_weight"] = "balanced"
            elif args.rf_use_sample_weight or (args.rf_use_class_weight and use_cuml):
            ...
            if monitor is not None:
                monitor.set_status(f"Training RF h={h} ({backend_name})")

            clf = RandomForestClassifier(**params)

            # cuML requires numpy arrays, sklearn accepts both
            if use_cuml:
                # Convert to numpy if needed, cuML doesn't accept lists
                Xk_np = Xk if isinstance(Xk, np.ndarray) else np.array(Xk)
                yk_np = yk if isinstance(yk, np.ndarray) else np.array(yk)
                if sample_weight is not None:
                    clf.fit(Xk_np, yk_np, sample_weight=sample_weight)
                else:
                    clf.fit(Xk_np, yk_np)
            else:
                # sklearn
        ...
        for k, h in enumerate(args.horizons):
            ...
            print(f"[Save] Random Forest ({backend_name}) h={h} → {path}")

        # Save backend info for loading later
        with open(os.path.join(args.logdir, "model_info.json"), "w") as f:
            json.dump({
