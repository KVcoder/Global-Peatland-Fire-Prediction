...
def main():
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if val_loader is not None:
            ...
            if math.isfinite(cur_ll) and cur_ll < best_val_logloss:
                ...
                best_path = os.path.join(args.logdir, "best_model.pt")  # keep same filename
                # or: best_path = os.path.join(args.logdir, "best_model_logloss.pt")
                ...
                torch.save(
                    {
                        ...
                        "val_f1": float(val_metrics.get("f1", float("nan"))),  # optional: keep for reference
                        "args": vars(args),
                        "calibration_method": calib_method,
                    },
                    best_path,
                )
                print(f"[Model] New best logloss={best_val_logloss:.6f} at epoch {epoch} -> {best_path}")
    ...
    if best_epoch > 0 and math.isfinite(best_val_logloss):
        print(f"  • Best val logloss............. {best_val_logloss:.6f} (epoch {best_epoch})")
    else:
        print("  • Best val logloss............. n/a")
