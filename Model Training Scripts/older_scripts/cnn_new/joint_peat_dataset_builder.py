...
- Supports an explicit val_frac in addition to split:
...
def start_dask_cluster(
    ...
    class _NoopCtx:
        def __enter__(self):
            # Best-effort: if dask is available, set a local scheduler.
            # If not installed, just no-op.
            try:
                import dask
                if scheduler == "threads":
                    ...
                    # Unknown or 'distributed' fallback: leave dask defaults
                    pass
            except ModuleNotFoundError:
                # Dask not installed at all; silently ignore.
                pass
            return None
...
class JointPeatDataset(Dataset):
    ...
    def __init__(
        ...
        self.C_era5 = int(C_e)
        self.C_smap = int(C_s)
        self.C_base = self.C_era5 + self.C_smap   # base meteo+soil channels
        self.H = int(H_e)
        self.W = int(W_e)
    ...
    def _make_coord_features_patch(self, y0: int, x0: int) -> np.ndarray:
        """
        Build coordinate feature patch of shape (4, patch, patch) with:
            [sin(lat), cos(lat), sin(lon), cos(lon)] in radians.
        """
        ...
        feats = np.stack([sin_lat, cos_lat, sin_lon, cos_lon], axis=0).astype(np.float32)
        return feats
    ...
    def _build_index(self):
        ...
        Supports:
          - 2-way split (train/val) when val_frac is None  [backwards compatible]
          - 3-way split (train/val/test) when val_frac is not None:
                train_frac = split
                val_frac   = val_frac
                test_frac  = 1 - split - val_frac
        """
    ...
    def __getitem__(self, i: int):
        ...
        x_np = np.concatenate([era5_np, smap_np], axis=1)

        # Optional coordinate features as extra channels
    ...
    def _estimate_input_stats(self, sample_count: int = 256):
        """
        Estimate per-channel mean/std of ERA5+SMAP(+coords) inputs from a subset of samples.
        """
        if len(self.index) == 0:
        ...
        for j in idxs:
            ...
            # include coord features if enabled
            if self.coord_as_features:
