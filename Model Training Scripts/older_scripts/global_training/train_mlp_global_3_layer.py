...
KB NOTES: Adding in another layer and more epochs didn't really provide more benefit, this test  was a failure 

we need to use less complex models

but actually, more epochs does help to a certain extent with the calibration. 6 epochs isn't enough but 25 might be too many
im not really sure. I think 12-15 epochs would probably be good.
...
def _choose_mp_context(requested: str) -> Optional[str]:
    ...
    # Respect what the user asked for (when possible)
    if requested == "fork":
        return "fork" 
...
def make_loader(ds, batch_size, shuffle, args, sampler=None, device: Optional[torch.device] = None):
...
class TileMLP3Layer(nn.Module):
    """Per-pixel independent MLP (3-layer)."""
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(self.in_features, self.hidden1)
        self.fc2 = nn.Linear(self.hidden1, self.hidden2)
        self.fc3 = nn.Linear(self.hidden2, self.K)

        def make_norm(d: int):
            if norm == "ln":
                return nn.LayerNorm(d, eps=float(bn_eps))
            if norm == "bn":
                return nn.BatchNorm1d(d, momentum=float(bn_momentum), eps=float(bn_eps))
            return nn.Identity()

        self.norm1 = make_norm(self.hidden1)
        self.norm2 = make_norm(self.hidden2)
    ...
    def _forward_pixels(self, x2: torch.Tensor) -> torch.Tensor:
        # x2: (N, F) -> (N, K)
        h = self.drop(self.act(self.norm1(self.fc1(x2))))
    ...
    def forward(self, batch: Dict[str, Any]) -> torch.Tensor:
        ...
        if self.chunk_tiles > 0 and x2.shape[0] > self.chunk_tiles:
            ...
            for i in range(0, x2.shape[0], self.chunk_tiles):
                outs.append(self._forward_pixels(x2[i : i + self.chunk_tiles]))
            out = torch.cat(outs, dim=0)  # (N,K)
        else:
            out = self._forward_pixels(x2)  # (N,K)

        return out.view(B, H, W, self.K).permute(0, 3, 1, 2).contiguous()  # (B,K,H,W)
...
def evaluate(
    ...
    criterion,
    args,
    calibrator: Optional[ProbCalibrator],
    use_tqdm: bool,
    logits_fn=None,   # <-- ADD THIS
):
    ...
    prob_hist_h = np.zeros((num_h, dist_bins), dtype=np.float64)  # used for approximate percentiles
        ...
        # ✅ NEW: observed target (VIIRS) stats on the same valid pixels
    tgt_pos_h = np.zeros(num_h, dtype=np.float64)      # sum of y (since y in {0,1})
    tgt_total_h = np.zeros(num_h, dtype=np.float64)    # total valid pixels (per horizon)
    ...
    for batch in TimedDataLoader(loader, desc="eval", use_tqdm=use_tqdm, position=3):
        ...
        logits = logits_fn(batch) if logits_fn is not None else model(batch)
        ...
        for k_idx in range(K_eff):
            ...
            # ✅ NEW: observed target stats (y_flat is 0/1 float here)
    ...
    # =========================
    # Step 2F: finalize target_stats (observed y)
    # =========================
    ...
    if tot_all <= 0:
        ...
        frac_all = pos_all / tot_all
        std_all = float(np.sqrt(max(frac_all * (1.0 - frac_all), 0.0)))
        mn_all_t = 1.0 if (tot_all - pos_all) == 0 else 0.0
        mx_all_t = 0.0 if pos_all == 0 else 1.0
        target_stats_overall = {
            "positive_fraction": float(frac_all),
            "positive_count": float(pos_all),
            "total_count": float(tot_all),
            "min": float(mn_all_t),
            "mean": float(frac_all),
            "max": float(mx_all_t),
            "std": float(std_all),
        }
        target_stats_overall.update(_binary_percentiles(pos_all, tot_all))
...
@torch.no_grad()
def evaluate_with_logits_fn(logits_fn, loader, device, criterion, args, calibrator=None, use_tqdm=True):
    # reuse your evaluate() metric machinery
    return evaluate(
        model=None,
        loader=loader,
        device=device,
...
@torch.no_grad()
def evaluate_ensemble_logits_mean(ckpt_paths, loader, device, eval_criterion, args, use_tqdm=True):
    # infer in_channels from dataset sample (same logic as your main)
    ...
    def logits_fn(batch):
        ...
        return z / float(len(models))

    return evaluate_with_logits_fn(
        logits_fn=logits_fn,
        loader=loader,
        device=device,
        criterion=eval_criterion,
        args=args,
        calibrator=None,      # keep None unless you implement ensemble calibration
        use_tqdm=use_tqdm,
    )
...
def _print_split(epoch: int, split: str, loss: float, metrics: Dict[str, Any], calib_method: str):
    ...
    # ✅ NEW: per-horizon target stats + compare to predicted mean
    tper = (metrics.get("target_stats", {}) or {}).get("per_horizon", {}) or {}
    for h, hh in tper.items():
        fire_frac = hh.get("positive_fraction", float("nan"))
        pos = hh.get("positive_count", float("nan"))
        tot = hh.get("total_count", float("nan"))

        pred_mean = (per.get(h, {}) or {}).get("mean", float("nan"))
        delta = (pred_mean - fire_frac) if (isinstance(pred_mean, (int, float)) and isinstance(fire_frac, (int, float))) else float("nan")

        print(
            f"  [{split.capitalize()} target_stats h={h}] "
            f"fire_frac={fire_frac:.6g} (pos={pos:.0f}/{tot:.0f}) | "
            f"min={hh.get('min', float('nan')):.6g}, "
            f"mean={hh.get('mean', float('nan')):.6g}, "
            f"max={hh.get('max', float('nan')):.6g}, "
            f"std={hh.get('std', float('nan')):.6g} | "
        ...
        if math.isfinite(float(delta)):
            print(f"    [{split.capitalize()} compare h={h}] pred_mean({pred_mean:.6g}) - fire_frac({fire_frac:.6g}) = {delta:.6g}")


    # ✅ NEW: observed target stats + compare means
    ts = (metrics.get("target_stats", {}) or {}).get("overall", {}) or {}
    if ts:
        fire_frac = ts.get("positive_fraction", float("nan"))
        pos = ts.get("positive_count", float("nan"))
        tot = ts.get("total_count", float("nan"))

        pred_mean = ps.get("mean", float("nan")) if ps else float("nan")
        delta = (pred_mean - fire_frac) if (isinstance(pred_mean, (int, float)) and isinstance(fire_frac, (int, float))) else float("nan")

        print(
            f"[{split.capitalize()}] target_stats overall: "
            f"fire_frac={fire_frac:.6g} (pos={pos:.0f}/{tot:.0f}) | "
            f"min={ts.get('min', float('nan')):.6g}, "
            f"mean={ts.get('mean', float('nan')):.6g}, "
            f"max={ts.get('max', float('nan')):.6g}, "
            f"std={ts.get('std', float('nan')):.6g} | "
            f"p1={ts.get('p1', float('nan')):.6g}, "
            f"p5={ts.get('p5', float('nan')):.6g}, "
            f"p50={ts.get('p50', float('nan')):.6g}, "
            f"p95={ts.get('p95', float('nan')):.6g}, "
            f"p99={ts.get('p99', float('nan')):.6g}"
        )
        if math.isfinite(float(delta)):
            print(f"[{split.capitalize()}] compare: pred_mean({pred_mean:.6g}) - fire_frac({fire_frac:.6g}) = {delta:.6g}")
...
def build_datasets(args):
    ...
    if not args.input:
        raise ValueError("You must provide at least one --input store.zarr[:array]")

    inputs = [parse_input_spec(s) for s in args.input]
    viirs_spec = parse_input_spec(args.viirs_zarr)

    # --- Year anchor indices (for either pure time split OR spatial holdout in that year) ---
    ...
    if args.test_year is not None:
        if use_spatial_holdout:
            test_t_end_index = _anchor_t_ends_for_year(args, viirs_spec)
            ...
            # existing leak-proof time-only split
    ...
    common = dict(
        ...
        time_index=train_time_index,   # <-- NEW (None if not using year split)
        coord_as_features=True,
        return_coords=False,
        peat_mask_source="smap_wtd.zarr",
        coords_source="smap_wtd.zarr",  # if you want coords forced too
        coords_units="auto",
    ...
    common["coords_units"] = args.coords_units
    
    # keep defaults unless user passed explicit overrides
    if args.peat_mask_source.strip():
        common["peat_mask_source"] = args.peat_mask_source.strip()

    if args.coords_source.strip():
        common["coords_source"] = args.coords_source.strip()


    
    header("Building datasets")

    # Train/Val always come from the *train years* time_index (or None)
    ...
    if args.test_year is not None:
        ...
        if (not use_spatial_holdout) and (test_time_index is not None):
            # time-only leakproof split
            common_test["time_index"] = test_time_index
        else:
            # spatial-holdout: test is defined by (t_end in year) + region mask
        ...
        print("  • No test_year specified; skipping test dataset. THIS IS PRETTY BAD GO FIX IT")
        # Old behavior (fractional split) if you still want it:
...
def _load_time_years_from_viirs(viirs_zarr: str, viirs_array: str) -> np.ndarray:
    """
    Load years from the VIIRS zarr time coordinate, returning an int array of shape (T,).
    Tries common coord names. Requires xarray only if you use year splitting.
    """
    import xarray as xr

    ds = xr.open_zarr(viirs_zarr, consolidated=False)

    # try array as data_var first, else allow coord-only datasets
    if viirs_array in ds.data_vars:
        arr = ds[viirs_array]
    elif viirs_array in ds:
        arr = ds[viirs_array]
    else:
        # fallback: pick first data var
    ...
    t = np.asarray(t)
    if t.dtype.kind != "M":
        # If your time axis is not datetime64, you’ll need to adapt this conversion.
...
def _resolve_time_indices_by_test_year_anchor_leakproof(args, viirs_spec):
    ...
    years = _load_time_years_from_viirs(viirs_spec.zarr, viirs_spec.array)  # (T,)
    T = int(years.shape[0])
    ...
    # ✅ Test set defined by anchor (t_end) year == test_year
    ...
    # Reserve all timesteps touched by any test window: history + label times up to max_h
    ...
    # Optional override: restrict train/val/calib to specific years
    ...
    print("  • Note: test labels can be in 2024 if horizons push t_end+h into 2024.")

    return train_time_index, test_time_index, test_t_ends

def _anchor_t_ends_for_year(args, viirs_spec) -> list[int]:
    years = _load_time_years_from_viirs(viirs_spec.zarr, viirs_spec.array)  # (T,)
    T = int(years.shape[0])
    ...
    test_t_ends = [t for t in range(t_end_first, t_end_last + 1) if int(years[t]) == test_year]
    if not test_t_ends:
        raise RuntimeError(f"No valid t_end anchors found for test_year={test_year}.")
    return test_t_ends
...
def split_train_for_calibration(train_ds, args):
    ...
    rng = np.random.RandomState(args.data_seed + 1337)
    idx = np.arange(N)
    rng.shuffle(idx)
...
def parse_args():
    ...
    p.add_argument("--reliability-fine-bins-cap", type=int, default=250_000,
                help="Caps number of fine bins for memory safety. If exceeded, bin width is increased.")
    ...
    # Year-based split (optional)
    p.add_argument("--test-year", type=int, default=None,
                   help="If set, use this year as the test set (time-based).")
    p.add_argument("--train-years", type=str, default="",
                   help="Optional override for train/val/calib years, e.g. '2016-2022,2024'. "
                        "If omitted, uses all available years except --test-year.")

    p.add_argument(
        "--test-region-mask-source",
        type=str,
        default="",
        help="Optional. Zarr store with 'peat_mask' defining a held-out region (finite & >0 pixels = region). "
            "For anchors in --test-year, train/val/calib exclude this region and test uses ONLY this region.",
    )
    p.add_argument(
        "--test-region-min-fraction",
        type=float,
        default=0.01,
        help="Minimum fraction of a patch covered by the holdout region mask to count as 'in-region'.",
    )
    ...
    p.add_argument("--lr-min", type=float, default=1e-6, help="Minimum LR for cosine schedule.")
    p.add_argument("--lr-step-size", type=int, default=5, help="Epochs per decay step (StepLR).")
    p.add_argument("--lr-gamma", type=float, default=0.5, help="Decay factor (StepLR).")
    p.add_argument("--warmup-epochs", type=int, default=0, help="Linear warmup epochs before cosine.")
    p.add_argument("--warmup-start-factor", type=float, default=0.1,
                help="Warmup starts at (start_factor * lr) and ramps to lr.")
    ...
    p.add_argument("--seeds", type=str, default="42",
               help="Comma-separated seeds, e.g. '1,2,3,4' to train multiple models.")
    p.add_argument("--data-seed", type=int, default=123,
                help="Seed used ONLY for dataset splitting/calib split (kept fixed across model seeds).")
    p.add_argument("--ensemble", action="store_true",
                help="After training all seeds, evaluate an ensemble on val/test.")
    p.add_argument("--ensemble-mode", choices=["logits_mean", "probs_mean"], default="logits_mean",
                help="logits_mean is easiest to integrate with your current evaluate().")
    ...
    p.add_argument(
        ...
        type=float,
        default=0.06,
        help="(Deprecated for printing) Probability threshold expressed in percent. 0.06 means 0.06% => prob=0.0006. EDIT: THIS IS PROBABLY FALSE",
    )
...
def train_one_seed(
    ...
    in_channels: int,
    device: torch.device,
    train_weights: Optional[torch.Tensor] = None,   # for weighted sampling
) -> str:
    ...
    if args.lr_scheduler == "cosine":
        warm = int(getattr(args, "warmup_epochs", 0))
        warm = max(0, warm)

        if warm > 0:
            if warm >= int(args.epochs):
            ...
            scheduler = SequentialLR(
                ...
                milestones=[warm],   # switch after `warm` epochs
            )
        else:
    ...
    # sampling / loaders (create loaders per seed = fewer worker/persistent quirks)
    train_sampler = None
    if args.patch_sampling == "weighted":
        if train_weights is None:
            raise RuntimeError("train_weights is None but --patch-sampling weighted was requested.")
        # NOTE: this uses global torch RNG; seed already set -> different seeds will sample differently
        train_sampler = WeightedRandomSampler(train_weights, num_samples=len(train_weights), replacement=True)

    train_loader = make_loader(train_ds, batch_size=args.batch_size, shuffle=True, args=args, sampler=train_sampler, device=device)
    val_loader = make_loader(val_ds, batch_size=args.batch_size, shuffle=False, args=args, device=device) if val_ds is not None else None
    test_loader = make_loader(test_ds, batch_size=args.batch_size, shuffle=False, args=args, device=device) if test_ds is not None else None
    calib_loader = make_loader(calib_ds, batch_size=args.batch_size, shuffle=False, args=args, device=device) if calib_ds is not None else None

    monitor = RAMMonitor(device_id=(device.index or 0) if device.type == "cuda" else 0)
    if not args.no_tqdm:
        monitor.start()

    best_val_logloss, best_epoch = float("inf"), -1
    global_step = 0
    ...
    for epoch in range(1, int(args.epochs) + 1):
        if monitor:
            monitor.set(f"Train seed{args.seed} e{epoch}/{args.epochs}")
        ...
        # optional post-hoc calibration (per epoch, same as your current main)
        calibrator = None
        calib_method = "none"
        if calib_loader is not None and args.calibration_method != "none":
            calib_method = args.calibration_method
            if monitor:
                monitor.set(f"Calibrating seed{args.seed} ({calib_method}) e{epoch}/{args.epochs}")
            calibrator = fit_calibrator(model, calib_loader, device, args)

        if val_loader is not None:
            if monitor:
                monitor.set(f"Eval(val) seed{args.seed} e{epoch}/{args.epochs}")
            ...
            # history bookkeeping (same as your main)
            vh = history["val"]
            ...
            cur_ll = float(val_metrics.get("logloss", float("nan")))
            if math.isfinite(cur_ll) and cur_ll < best_val_logloss:
                best_val_logloss = cur_ll
                best_epoch = epoch
                ...
                print(f"[Model] seed{args.seed} new best logloss={best_val_logloss:.6f} at epoch {epoch} -> {best_path}")

        if epoch == int(args.epochs) and test_loader is not None:
            if monitor:
                monitor.set(f"Eval(test) seed{args.seed} e{epoch}/{args.epochs}")
        ...
        if scheduler is not None:
            ...
            print(f"[LR] epoch={epoch} lr={cur_lr:.6g}")
            if writer is not None:
    ...
    if not os.path.exists(best_path):
        raise RuntimeError(f"Expected best checkpoint at {best_path} but it wasn't created.")
    return best_path
...
def main():
    ...
    # Decide whether to run multi-seed mode (and ensemble)
    seeds = parse_seeds_list(args.seeds)
    multi_seed_requested = (len(seeds) > 1) or ("," in (args.seeds or "")) or bool(args.ensemble)

    # ---- build datasets ONCE (split controlled by args.data_seed inside build_datasets) ----
    monitor = RAMMonitor(device_id=(device.index or 0) if device.type == "cuda" else 0)
    if not args.no_tqdm:
        monitor.start()
    ...
    print(f"  • seeds....................... {seeds}")
    print(f"  • multi-seed mode............. {multi_seed_requested}")
    print(f"  • ensemble eval............... {bool(args.ensemble)}")
    print(f"  • ensemble mode............... {args.ensemble_mode}")

    # ---- If NOT multi-seed, keep your old single-seed training loop ----
    if not multi_seed_requested:
        # Your existing single-seed main training path (UNCHANGED) lives below.
        # (I’m leaving it intact: you already have it.)
        #
        # NOTE: If you prefer, you can also just call train_one_seed(...) here and
        # accept writing into runs/.../seed{seed}/ even for single seed.
        #
        # >>> KEEP YOUR CURRENT SINGLE-SEED CODE HERE <<<
        #
        # For brevity, I’m delegating to train_one_seed so you don’t maintain two paths.
        header("Single-seed training (delegated to train_one_seed)")
        base_args = copy.deepcopy(args)
        ckpt_paths = []
        # optional precompute weights once
        train_weights = None
        if args.patch_sampling == "weighted":
            header("Patch sampling")
            print("  • Building WeightedRandomSampler weights (once, shared across seeds)...")
            train_weights = compute_patch_weights(
                train_ds,
                pos_weight=float(args.patch_pos_weight),
                max_items=args.patch_weight_scan_max,
            )
            pos_frac = float((train_weights.numpy() > 1.0).mean())
        ...
        header("Training complete (single seed)")
        print(f"  • best checkpoint............. {ckpt}")
        return  # (no ensemble unless requested; but multi_seed_requested would be True if --ensemble)

    # ---- Multi-seed training loop (writes per-seed folders) ----
    base_args = copy.deepcopy(args)
    base_logdir = base_args.logdir  # keep root

    # Optional: compute patch weights ONCE and pass into each seed training
    train_weights = None
    if args.patch_sampling == "weighted":
        header("Patch sampling")
        print("  • Building WeightedRandomSampler weights (once, shared across seeds)...")
    ...
    header("Finished training all seeds")
    for p in ckpt_paths:
        print(f"  • {p}")
    ...
    # put ensemble outputs in root/ensemble
    ens_args = copy.deepcopy(args)
    ens_args.logdir = os.path.join(base_logdir, "ensemble")
    os.makedirs(ens_args.logdir, exist_ok=True)

    header("Ensemble evaluation")
    print(f"  • ensemble logdir............. {ens_args.logdir}")
    print(f"  • checkpoints.................. {len(ckpt_paths)}")

    # build val/test loaders once for ensemble eval
    val_loader = make_loader(val_ds, batch_size=ens_args.batch_size, shuffle=False, args=ens_args) if val_ds is not None else None
    test_loader = make_loader(test_ds, batch_size=ens_args.batch_size, shuffle=False, args=ens_args) if test_ds is not None else None

    eval_loss_name = ens_args.eval_loss or ens_args.loss
    eval_criterion = make_criterion(eval_loss_name, ens_args).to(device)

    writer_ens: Optional[SummaryWriter] = None
    if not ens_args.no_tensorboard:
        writer_ens = SummaryWriter(log_dir=ens_args.logdir)
        print(f"[TB] Ensemble logging to: {ens_args.logdir}")

    metrics_csv_path = init_metrics_csv(ens_args.logdir)

    def _eval_ensemble(loader, split_name: str):
        if loader is None:
            return None, None

        if ens_args.ensemble_mode == "logits_mean":
            loss, metrics = evaluate_ensemble_logits_mean(
                ckpt_paths=ckpt_paths,
                loader=loader,
                device=device,
                eval_criterion=eval_criterion,
                args=ens_args,
                use_tqdm=not ens_args.no_tqdm,
            )
        elif ens_args.ensemble_mode == "probs_mean":
            # average probabilities, convert back to logits so evaluate() can reuse BCEWithLogits etc.
            ...
            if x0.dim() == 4:
                ...
                raise ValueError(f"Unexpected x shape: {tuple(x0.shape)}")
            ...
            for pth in ckpt_paths:
                ...
                m = TileMLP2Layer(
                    in_features=in_ch,
                    horizons=ens_args.horizons,
                    hidden=ens_args.mlp_hidden,
                    dropout=float(ens_args.dropout),
                    chunk_tiles=ens_args.mlp_chunk_tiles,
                    norm=ens_args.mlp_norm,
                    bn_momentum=ens_args.mlp_bn_momentum,
                    bn_eps=ens_args.mlp_bn_eps,
                ).to(device)
                m.load_state_dict(ckpt["model_state_dict"], strict=True)
                m.eval()
                models.append(m)
            ...
            def logits_fn(batch):
                p_sum = None
                for m in models:
                    p = torch.sigmoid(m(batch))
                    p_sum = p if p_sum is None else (p_sum + p)
                p_mean = p_sum / float(len(models))
                p_mean = p_mean.clamp(1e-6, 1.0 - 1e-6)
                return torch.log(p_mean) - torch.log(1.0 - p_mean)

            loss, metrics = evaluate_with_logits_fn(
                logits_fn=logits_fn,
                loader=loader,
                device=device,
                criterion=eval_criterion,
            ...
            raise ValueError(f"Unknown ensemble_mode: {ens_args.ensemble_mode}")

        # print + csv + plots/artifacts
        _print_split(epoch=0, split=split_name, loss=loss, metrics=metrics, calib_method="ensemble")
        append_metrics_csv(metrics_csv_path, 0, split_name, loss, metrics, "ensemble")
        save_and_log_calibration(split_name, metrics, 0, ens_args, writer_ens)
        return loss, metrics
    ...
    if val_loader is not None:
        _, last_val_metrics = _eval_ensemble(val_loader, "val_ensemble")
    if test_loader is not None:
        _, final_test_metrics = _eval_ensemble(test_loader, "test_ensemble")

    # optional plots
    try:
        if ens_args.plot_metrics:
            if last_val_metrics is not None:
                plot_roc_curves_from_metrics(last_val_metrics, ens_args, split="val_ensemble", epoch=0)
            if final_test_metrics is not None:
                plot_roc_curves_from_metrics(final_test_metrics, ens_args, split="test_ensemble", epoch=0)
    except Exception as e:
        print(f"[Plots] Failed to generate ensemble ROC plots: {e}")

    if writer_ens is not None:
        writer_ens.flush()
        writer_ens.close()
