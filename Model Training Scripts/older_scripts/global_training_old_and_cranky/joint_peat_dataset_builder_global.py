...
class JointPeatDataset(Dataset):
    ...
    exclude_region_mask_source=None,
    exclude_region_min_frac=0.01,
    exclude_region_year=None,
    exclude_region_leakproof_window=False,
    exclude_region_mode="none",  # "none" | "drop_in_year" | "keep_only_region_year"
        ...
        self.lat_grid = None
        self.lon_grid = None
        self.years = _load_time_years_from_viirs(viirs_zarr, viirs_array)

        
        self.region_mask = None
        if exclude_region_mask_source:
            self.region_mask = self._load_region_mask(exclude_region_mask_source)
        self.exclude_region_min_frac = float(exclude_region_min_frac)
        self.exclude_region_year = int(exclude_region_year) if exclude_region_year is not None else None
        self.exclude_region_leakproof_window = bool(exclude_region_leakproof_window)
        self.exclude_region_mode = str(exclude_region_mode or "none")
    ...
    def _load_region_mask(self, zarr_path: str):
        """
        Load a region mask from a zarr on the global grid.
        You said this store has valid peat-mask values only in SE Asia.
        We interpret region := finite & >0 (adjust if your variable differs).
        """
        import xarray as xr
        ds = xr.open_zarr(zarr_path, consolidated=False)

        # Try common names; else pick first data var
        if "peat_mask" in ds:
            m = ds["peat_mask"]
        elif "field" in ds:
            m = ds["field"]
        else:
            m = next(iter(ds.data_vars.values()))

        arr = m.values
        region = np.isfinite(arr) & (arr > 0)
        return region.astype(np.uint8)  # (H,W)

    def _patch_region_frac(self, y0: int, x0: int, patch: int) -> float:
        if self.region_mask is None:
            return 0.0
        sub = self.region_mask[y0:y0+patch, x0:x0+patch]
        if sub.size == 0:
            return 0.0
        return float(sub.mean())

    def _window_intersects_year(self, t_end: int, year: int) -> bool:
        """
        True if [t_end-(t_hist-1), t_end+max_h] intersects given year.
        Requires self.years as int array (T,).
        """
        max_h = max(self.horizons) if self.horizons else 0
        t0 = max(0, t_end - (self.t_hist - 1))
        t1 = min(len(self.years) - 1, t_end + max_h)
        return bool((self.years[t0:t1+1] == year).any())
    ...
    def _build_index(self):
        ...
        # ============================================================
        # B5) Region+Year exclusion/selection (SE Asia 2023 test-only)
        # Insert this block RIGHT HERE (before shuffle/split).
        # ============================================================
        if (
            self.region_mask is not None
            and self.exclude_region_year is not None
            and self.exclude_region_mode != "none"
        ):
            yr = int(self.exclude_region_year)

            def _is_region_patch(y0: int, x0: int) -> bool:
                return self._patch_region_frac(y0, x0, self.patch) >= float(self.exclude_region_min_frac)

            # For train/val/calib we usually want "leak-proof" window exclusion:
            # drop if ANY part of [t_end-(t_hist-1), t_end+max_h] touches year=yr.
            def _year_hit_for_drop(t_end: int) -> bool:
                if self.exclude_region_leakproof_window:
                    return self._window_intersects_year(t_end, yr)
                return bool(self.years[t_end] == yr)

            # For test selection you typically want *anchor-year* == yr (t_end year),
            # otherwise you may accidentally include 2022 anchors whose horizon spills into 2023.
            def _year_hit_for_keep(t_end: int) -> bool:
                return bool(self.years[t_end] == yr)

            before = len(full_index)

            if self.exclude_region_mode == "drop_in_year":
                full_index = [
                    (t_end, y0, x0)
                    for (t_end, y0, x0) in full_index
                    if not (_is_region_patch(y0, x0) and _year_hit_for_drop(t_end))
                ]

            elif self.exclude_region_mode == "keep_only_region_year":
                full_index = [
                    (t_end, y0, x0)
                    for (t_end, y0, x0) in full_index
                    if (_is_region_patch(y0, x0) and _year_hit_for_keep(t_end))
                ]

            else:
                raise ValueError(f"Unknown exclude_region_mode: {self.exclude_region_mode}")

            after = len(full_index)
            print(
                f"[exclude_region] mode={self.exclude_region_mode} year={yr} "
                f"min_frac={self.exclude_region_min_frac} leakproof={self.exclude_region_leakproof_window} "
                f"kept={after}/{before} (dropped={before-after})"
            )

            if not full_index:
                raise RuntimeError(
                    "After exclude_region filtering, no samples remain. "
                    "Check exclude_region_min_frac/year/mode."
                )
