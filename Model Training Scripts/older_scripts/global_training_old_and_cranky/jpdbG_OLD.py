...
class JointPeatDataset(Dataset):
    ...
    def _build_index(self):
        ...
        if self.t_end_index is not None:
            t_ends = [int(t) for t in self.t_end_index]
            t_ends = [t for t in t_ends if (t_end_first <= t <= t_end_last)]
        else:
        ...
        if not t_ends:
            raise RuntimeError("No valid t_end anchors after applying t_end_index/time_index/bounds.")
        ...
        if self.skip_nonpeat_patches and self.peat_mask is not None:
            ...
            for (y0, x0) in spatial:
                patch_mask = (self.peat_mask[y0:y0 + self.patch, x0:x0 + self.patch] > 0)
                frac_peat = float(patch_mask.mean())
