...
from joint_peat_dataset_builder_global import JointPeatDataset
...
class TileMLP2Layer(nn.Module):
    ...
    def __init__(
        ...
        if norm == "ln":
            # LayerNorm is per-sample (per pixel vector), stable under chunking
            self.norm1 = nn.LayerNorm(self.hidden, eps=float(bn_eps))
        elif norm == "bn":
            # BatchNorm uses batch/chunk statistics (can vary with chunking)
...
def _resolve_test_t_end_anchors(args, viirs_spec):
    ...
    if not test_t_ends:
        raise RuntimeError(f"No valid test t_end anchors found for test_year={test_year}.")

    header("Test anchors (no global time exclusion)")
    print(f"  • Test anchor year............ {test_year}")
    print(f"  • # test t_end anchors......... {len(test_t_ends)}")
    print("  • Train/val/calib keep all years globally; exclusion is applied only inside the region mask.")
...
def build_datasets(args):
    from joint_peat_dataset_builder_global import JointPeatDataset, parse_input_spec
    ...
    test_t_ends = _resolve_test_t_end_anchors(args, viirs_spec)  # <-- NEW
    ...
    common = dict(
        ...
        time_index=None,  # <-- IMPORTANT: NO global year/time exclusion
        ...
        # --- NEW: region-mask-based filtering inputs ---
        exclude_region_mask_source=(args.exclude_region_mask_source.strip() or None),
        exclude_region_min_frac=float(args.exclude_region_min_frac),
        exclude_region_year=(int(args.exclude_region_year) if args.exclude_region_mask_source.strip() else None),
        exclude_region_leakproof_window=bool(args.exclude_region_leakproof_window),
        exclude_region_mode="drop_in_year",  # train/val/calib behavior
    )
    ...
    header("Building datasets (global train; region-year excluded only inside mask)")
    ...
    # Test:
    # If year split is enabled, force test to use ONLY test_year time_index and include ALL of it.
    test_ds = None
    if test_t_ends is not None:
        common_test = dict(common)
        common_test["split"] = 0.0
        common_test["val_frac"] = 0.0
        common_test["t_end_index"] = test_t_ends   # <-- anchor-year test definition
        common_test["exclude_region_mode"] = "keep_only_region_year"  # <-- test = region+year
        common_test["exclude_region_year"] = int(args.test_year)      # <-- test year
        test_ds = JointPeatDataset(mode="test", **common_test)
        print(f"  • Test patches (leak-proof).. {len(test_ds)}")

    else:
        "YEAR SPLIT IS DISABLED: using standard split for test set (HARD FLUCKIN ERROR GO FIX RN BOMBOCLAT)"
    ...
    if test_ds is not None:
        print(f"  • Test patches (SE Asia {args.test_year}).. {len(test_ds)}")
...
from joint_peat_dataset_builder_global import _load_time_years_from_viirs
...
def parse_args():
    ...
    p.add_argument("--peat-mask-source", type=str, default="smap_wtd_SE_Asia_t32_p32.zarr")
    p.add_argument("--coords-source", type=str, default="smap_wtd_SE_Asia_t32_p32.zarr")
    ...
    # --- SE Asia 2023 exclusion (spatiotemporal) ---
    p.add_argument("--exclude-region-mask-source", type=str, default="",
                help="Zarr store on global grid whose peat/valid values exist only in SE Asia. "
                        "Used to define a region mask for exclusion/test.")
    p.add_argument("--exclude-region-year", type=int, default=2023,
                help="Year to exclude inside exclude-region-mask-source for train/val/calib.")
    p.add_argument("--exclude-region-min-frac", type=float, default=0.01,
                help="A patch is considered 'inside the region' if >= this fraction overlaps the region mask.")
    p.add_argument("--exclude-region-leakproof-window", action="store_true",
                help="If set, drop any SE Asia patch whose [t_end-(T_hist-1), t_end+max_h] intersects exclude year.")
