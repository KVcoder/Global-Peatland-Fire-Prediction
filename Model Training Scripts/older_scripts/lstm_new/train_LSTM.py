...
# ----------------------------------------------------------------------
# LSTM baseline over time-per-pixel
# ----------------------------------------------------------------------

class LSTMSpatiotemporal(nn.Module):
    """
    Simple LSTM over time at each spatial location.

    Expects batch["x"] as either:
      - (B, T, C, H, W)   when time_stack="separate"
      - (B, T*C, H, W)    when time_stack="channel", with T = t_hist, C = feature_dim

    For each pixel (h, w), we run an LSTM over the T-step sequence of C-dim
    features, then map the final hidden state to K horizon logits. These are
    reshaped back to (B, K, H, W) so that the rest of the pipeline (loss,
    metrics, masks) remains unchanged.
    """
    def __init__(
        self,
        feature_dim: int,
        ...
        head_in_dim = self.hidden_size * self.num_directions
        self.horizon_heads = nn.ModuleList(
            [nn.Linear(head_in_dim, 1) for _ in self.horizons]
        )
    ...
    def _to_B_T_C(self, x: torch.Tensor) -> torch.Tensor:
        """
        Normalize x to shape (B, T, C, H, W).
        """
        if x.dim() == 5:
            # Assume (B, T, C, H, W)
            B, T, C, H, W = x.shape
            ...
            if self.feature_dim <= 0 or self.t_hist <= 0:
                raise ValueError(
                    f"LSTMSpatiotemporal: invalid feature_dim={self.feature_dim} or t_hist={self.t_hist}"
                )
            ...
            if T != self.t_hist:
                # Trust the runtime data more than the config
                self.t_hist = T
            x = x.view(B, T, self.feature_dim, H, W)
            return x
        else:
            raise ValueError(
                f"LSTMSpatiotemporal expected x with 4 or 5 dims, got {tuple(x.shape)}"
            )
    ...
    def forward(self, batch: Dict[str, Any]) -> torch.Tensor:
        ...
        x = self._to_B_T_C(x)           # (B, T, C, H, W)
        B, T, C, H, W = x.shape

        # Reorder to treat each spatial location as a separate sequence:
        # (B, H, W, T, C) -> (B*H*W, T, C)
        x = x.permute(0, 3, 4, 1, 2).contiguous()
        B_, H_, W_, T_, C_ = x.shape
        assert B_ == B and H_ == H and W_ == W and T_ == T and C_ == C
        x = x.view(B * H * W, T, C)

        lstm_out, _ = self.lstm(x)      # (B*H*W, T, hidden*dirs)
        last = lstm_out[:, -1, :]       # take final time step: (B*H*W, hidden*dirs)

        logits_per_h = []
        for head in self.horizon_heads:
            logit = head(last)               # (B*H*W, 1)
            logit = logit.view(B, H, W, 1)   # (B, H, W, 1)
            logit = logit.permute(0, 3, 1, 2).contiguous()  # (B, 1, H, W)
            logits_per_h.append(logit)

        logits = torch.cat(logits_per_h, dim=1)  # (B, K, H, W)
        return logits
