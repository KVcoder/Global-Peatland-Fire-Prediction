...
- Handles NaNs/Infs in ERA5/SMAP inputs (nan_to_num -> 0).
- Masks out invalid VIIRS labels (NaNs) and peat-only label regions.
...
- OPTIONAL: append sin/cos(lat, lon) as extra feature channels in x
  (good for CNNs to handle longitude wrap-around cleanly).

NEW:
- Coordinates loader now supports x/y fallback:
...
class JointPeatDataset(Dataset):
    ...
    Per sample:
        "x":
           if time_stack="separate": (t_hist, C_total, patch, patch)
           if time_stack="channel" : (t_hist*C_total, patch, patch)
           where
             C_total = C_era5 + C_smap (+ 4 if coord_as_features=True)

             When coord_as_features=True, we append:
               sin(lat), cos(lat), sin(lon), cos(lon)
             as 4 *static* channels (same for every time step).
    ...
    def __init__(
        ...
        # These are filled once coords are loaded
        ...
        # Base feature count (ERA5+SMAP) and total including coord features
        self.base_C_total = self.C_era5 + self.C_smap
        self.C_total = self.base_C_total + (4 if self.coord_as_features else 0)
        ...
        # Needed if we either want coords tensor OR to embed sin/cos into x.
        if self.return_coords or self.coord_as_features:
            self._load_lat_lon_with_xy_fallback(era5_zarr, smap_zarr, viirs_zarr)
    ...
    def _load_peat_mask(self, era5_zarr: str, smap_zarr: str, viirs_zarr: str):
        ...
        if mask is None:
            ...
            if mask is not None:
                print("peat_mask loaded from ERA5 store.")
        if mask is None:
            mask = try_open_mask(viirs_zarr)
            if mask is not None:
                print("peat_mask loaded from VIIRS store.")
    ...
    def _load_lat_lon_with_xy_fallback(self, era5_zarr: str, smap_zarr: str, viirs_zarr: str):
        """
        ...
        2. If not found, try 'y' and 'x' arrays:
             ...
             - If 2D: use directly
        3. Assume units are degrees; convert to radians when needed.
        """
        ...
        # Step 2: if lat/lon missing, fallback to y/x
        if lat is None or lon is None:
            ...
            for root in (era5_zarr, smap_zarr, viirs_zarr):
                ...
                if y_arr is not None and x_arr is not None:
                    # Treat y as latitude, x as longitude in degrees
                    try:
        ...
        if lat is None or lon is None:
            raise RuntimeError(
                "return_coords/coord_as_features requested but could not find usable coordinate arrays.\n"
    ...
    def __getitem__(self, i: int):
        ...
        # Robust NaN / Inf handling for inputs
        era5_np = np.nan_to_num(era5_np, nan=0.0, posinf=0.0, neginf=0.0)
        smap_np = np.nan_to_num(smap_np, nan=0.0, posinf=0.0, neginf=0.0)

        x_np = np.concatenate([era5_np, smap_np], axis=1)  # (T_hist, base_C_total, H, W)

        # ---- NEW: append sin/cos(lat/lon) as static features, if requested ----
        if self.coord_as_features:
            if self.lat_grid is None or self.lon_grid is None:
                raise RuntimeError(
                    "coord_as_features=True but lat_grid/lon_grid not loaded. "
            ...
            # (4, H, W)
            coord_feats = np.stack([sin_lat, cos_lat, sin_lon, cos_lon], axis=0)

            # Make them static in time: repeat along time axis
            coord_feats_time = np.broadcast_to(
                coord_feats[None, ...], (self.t_hist, 4, self.patch, self.patch)
            )

            # Concatenate along channel dimension
            x_np = np.concatenate([x_np, coord_feats_time], axis=1)
        ...
        if self.time_stack == "channel":
            # (T_hist, C_total, H, W) -> (C_total * T_hist, H, W)
        ...
        for k, h in enumerate(self.horizons):
            ...
            # Safely cast to float32 and mask out any garbage
            # (NaN / ±inf / out-of-range) as "no label".
            with np.errstate(over="ignore", invalid="ignore"):
                lbl = np.asarray(slab[0, 0], dtype=np.float32)

            # Valid labels are:
            #   - finite
            #   - within [0, 1] (since VIIRS is binary 0/1)
        ...
        # ---------- Coordinates (lat/lon in radians, optional separate tensor) ----------
        coords = None
    ...
    def _estimate_input_stats(self, sample_count: int = 256):
        """
        Estimate per-channel mean/std of ERA5+SMAP (+optional coord features) from a subset of samples.
        """
        if len(self.index) == 0:
        ...
        for j in idxs:
            ...
            # Apply same coord feature logic as in __getitem__
            if self.coord_as_features:
                if self.lat_grid is None or self.lon_grid is None:
                    raise RuntimeError(
                        "coord_as_features=True but lat_grid/lon_grid not loaded during stats estimation."
                    )
                lat_patch = self.lat_grid[y0: y0 + self.patch, x0: x0 + self.patch]
                lon_patch = self.lon_grid[y0: y0 + self.patch, x0: x0 + self.patch]
            ...
            x_torch = torch.from_numpy(x_np).float()  # (T_hist, C_total, H, W)

            # Bring channels first: (C_total, T_hist, H, W)
...
def _example():
    ...
    parser = argparse.ArgumentParser(description="Joint ERA5+SMAP→VIIRS Zarr dataset (with x/y coord fallback)")
    ...
    parser.add_argument(
        "--coord-as-features",
        action="store_true",
        help="Append sin/cos(lat, lon) as extra static channels in x",
    )
