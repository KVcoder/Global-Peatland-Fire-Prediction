...
- UNet sees time stacked into channels, including these coord features.
...
from torch.utils.data import DataLoader, WeightedRandomSampler  # NEW
from tqdm.auto import tqdm
from torch.utils.tensorboard import SummaryWriter

from joint_peat_dataset_builder import JointPeatDataset  # <-- latest dataset with coord_as_features
...
def default(val, d): return d if val is None else d
...
class MaskedBCEWithLogits(nn.Module):
    ...
    """
    def __init__(self, enable_class_weights: bool = False, max_pos_weight: float = 100.0):  # NEW
        super().__init__()
    ...
    def forward(self, logits, targets, mask):
        ...
        if self.enable_class_weights:  # NEW: dynamic reweighting
            with torch.no_grad():
                valid = mask > 0.5
...
class MaskedFocalLossWithLogits(nn.Module):
    ...
    def __init__(self, alpha: float = 0.75, gamma: float = 2.0,
                 enable_class_weights: bool = False, max_pos_weight: float = 100.0):  # NEW
        super().__init__()
...
def evaluate(model, loader, device, criterion, args, use_tqdm: bool = True, show_worker_stats: bool = False):
    ...
    # Put validation progress bar on its own line (position=3)
    iterator = wrap_loader(loader, desc="val", use_tqdm=use_tqdm, show_worker_stats=show_worker_stats, position=3)
...
def parse_args():
    ...
    # --- Patch-level rebalancing & curriculum (NEW) ---
    p.add_argument("--patch-sampling", choices=["none", "balanced"], default="none",
                   help="Patch-level sampling for training. 'balanced' oversamples fire patches and undersamples fire-free ones.")
    ...
    p.add_argument("--curriculum-start-epoch", type=int, default=1,
                   help="Epoch at which to START ramping in non-fire patches (1-indexed). "
                        "Epochs before this are effectively fire-only.")
    p.add_argument("--curriculum-neg-weight-min", type=float, default=1e-3,
                   help="Non-fire patch weight at the START of the curriculum (very small ~ fire-only).")
...
def build_train_sampler(patch_fire_flags: Optional[np.ndarray], args, epoch: int):
    ...
    Curriculum semantics:
      ...
      Let:
        S = args.curriculum_start_epoch
        N = args.curriculum_epochs
        E = epoch

      If N <= 0:
        - No curriculum; only patch_sampling controls behaviour.

      Else (N > 0):
        - For E < S:
            * Fire-only (non-fire weight ~ 0).
        - For S <= E <= S + N - 1:
            * Progressive ramp:
                progress = 0 → 1 across these N epochs
                pos_weight ~ (balanced oversample factor)
                neg_weight = curriculum_neg_weight_min + progress * (max - min)
        - For E > S + N - 1:
            * Post-curriculum:
                - if patch_sampling == "balanced": balanced oversampling
                - else: default sampling (None).
    """
    ...
    # basic oversampling ratio for fire patches
    ...
    if epoch <= end_epoch:
        ...
        sampler = WeightedRandomSampler(
            torch.as_tensor(weights, dtype=torch.double),
            num_samples=n,  # draw N samples per epoch
            replacement=True,
        )
...
def print_env_and_cfg(args, device, train_ds, val_ds, model):
    ...
    print_diagnostic_item("Patch sampling", args.patch_sampling, indent=1)  # NEW
    print_diagnostic_item("Curriculum epochs", args.curriculum_epochs, indent=1)  # NEW
    print_diagnostic_item("Curriculum start epoch", args.curriculum_start_epoch, indent=1)  # NEW
    print_diagnostic_item("Curriculum neg_weight_min", args.curriculum_neg_weight_min, indent=1)  # NEW
    print_diagnostic_item("Curriculum neg_weight_max", args.curriculum_neg_weight_max, indent=1)  # NEW
    print_diagnostic_item("Pixel class weights", "on" if args.enable_pixel_class_weights else "off", indent=1)  # NEW
...
def test_first_batch(train_loader, device, model, args):
    ...
    criterion = MaskedBCEWithLogits()  # simple loss for diag
    t_start = time.time()
    with amp_autocast(device):
        loss = criterion(logits, batch["y"], batch["mask"])
    loss.backward()
...
def make_datasets(args):
    ...
    # For UNet baseline:
    #  - DO append sin/cos(lat,lon) as features (coord_as_features=True)
...
def main():
    ...
    # VAL-TEST-ONLY: run one validation pass and exit, with per-horizon stats
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            # Wrap loader for detailed timing, but without its own tqdm
            ...
            for batch_idx, batch in enumerate(timed_loader):
                ...
                # Queue stats disabled for now (avoid consuming a second iterator)
