...
def _maybe_run_fast_hpo_replot_from_csv_from_cli() -> None:
    """Regenerate HPO plots from a previously saved plot-data CSV and exit.

    This avoids importing torch/xgboost or rebuilding any dataset.
    """
    if "--hpo-replot-from-csv" not in sys.argv:
        return

    replot_parser = argparse.ArgumentParser(add_help=True)
    replot_parser.add_argument("--hpo-replot-from-csv", type=str, required=True)
    replot_parser.add_argument("--logdir", default=None)
    replot_parser.add_argument("--hpo-visualize-params", type=str, default="eta,max_depth")
    replot_parser.add_argument("--hpo-visualize-metric", type=str, default="log_loss")
    replot_parser.add_argument("--hpo-objective-metric", type=str, default="logloss")
    replot_parser.add_argument("--hpo-parallel-params", type=str,
                               default="eta,max_depth,min_child_weight,subsample,colsample_bynode,gamma,num_round")
    replot_parser.add_argument("--plot-file-format", type=str, default="png")
    replot_parser.add_argument("--plot-dpi", type=int, default=850)
    replot_parser.add_argument("--hpo-landscape-grid-size", type=int, default=160)
    replot_args, _ = replot_parser.parse_known_args()
    ...
    csv_path = os.path.abspath(replot_args.hpo_replot_from_csv)
    df = _pd.read_csv(csv_path)
    if df.empty:
        print(f"[HPO Replot] CSV has no rows: {csv_path}")
        raise SystemExit(1)

    metric = str(replot_args.hpo_visualize_metric or "log_loss")
    aliases = {"brier": "brier_score", "logloss": "log_loss"}
    metric_key = aliases.get(metric, metric)

    if metric_key not in df.columns:
        if metric_key == "value" and "value" in df.columns:
            pass
        else:
            print(f"[HPO Replot] Metric '{metric}' not found in CSV columns: {list(df.columns)}")
            raise SystemExit(1)

    df["plot_value"] = _pd.to_numeric(df[metric_key if metric_key in df.columns else "value"], errors="coerce")

    if "trial_number" not in df.columns:
        df["trial_number"] = _np.arange(len(df))

    out_root = replot_args.logdir
    if out_root is None:
        out_root = (
            os.path.dirname(os.path.dirname(csv_path))
            if os.path.basename(os.path.dirname(csv_path)) == "hpo_visualizations"
            else os.path.dirname(csv_path)
        )

    out_dir = os.path.join(out_root, "hpo_visualizations")
    os.makedirs(out_dir, exist_ok=True)

    def _metric_label(name):
        key = str(name)
        if key == "value":
            aliases2 = {"brier": "brier_score", "logloss": "log_loss"}
            key = aliases2.get(
                str(replot_args.hpo_objective_metric).strip().lower(),
                str(replot_args.hpo_objective_metric)
            )

        labels = {
            "value": "Validation log loss (lower is better)",
            "brier_score": "Validation Brier score (lower is better)",
            "brier": "Validation Brier score (lower is better)",
            "log_loss": "Validation log loss (lower is better)",
            "logloss": "Validation log loss (lower is better)",
            "ece": "Validation expected calibration error / ECE (lower is better)",
            "mce": "Validation maximum calibration error / MCE (lower is better)",
        }
        return labels.get(str(key), f"{key} (lower is better)")

    metric_label = _metric_label(metric)

    def _parse_pairs(spec):
        chunks = [c.strip() for c in str(spec or "").split(";") if c.strip()]
        if not chunks:
            return [("eta", "max_depth")]
    ...
    metadata = {
        ...
        "hpo_visualize_param_pairs": [[x, y] for (x, y) in param_pairs],
        "hpo_parallel_params": [x.strip() for x in str(replot_args.hpo_parallel_params).split(",") if x.strip()],
        "plot_file_format": replot_args.plot_file_format,
        "plot_dpi": int(replot_args.plot_dpi),
        "hpo_landscape_grid_size": int(replot_args.hpo_landscape_grid_size),
    }

    metadata_path = os.path.join(out_dir, f"{stem}_replot_metadata.json")
    with open(metadata_path, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=2)
    made.append(metadata_path)

    # ------------------------------------------------------------------
    # Parallel-coordinate plot
    # ------------------------------------------------------------------
    pnames = [
        p for p in [x.strip() for x in str(replot_args.hpo_parallel_params).split(",") if x.strip()]
        if p in df.columns
    ]
    ...
    for name in pnames:
        vals, label = _plot_vals(df[name], name, for_parallel=True)
        mn, mx = float(_np.nanmin(vals)), float(_np.nanmax(vals))

        if _np.isfinite(mn) and _np.isfinite(mx) and mx > mn:
            normed.append((vals - mn) / (mx - mn))
            labels.append(label)  # no hyperparameter ranges on x-axis

    if len(normed) >= 2:
        Y = _np.vstack(normed).T
        xs = _np.arange(Y.shape[1])

        fig, ax = _plt.subplots(figsize=(max(11.5, 1.9 * Y.shape[1]), 7.2))

        norm = _plt.Normalize(df["trial_number"].min(), df["trial_number"].max())
        ...
        for i in range(Y.shape[0]):
            ax.plot(
                xs,
                Y[i],
                color=cmap(norm(df.iloc[i]["trial_number"])),
                alpha=0.18 + 0.82 * (i / max(Y.shape[0] - 1, 1)),
                linewidth=1.1,
                zorder=1,
            )

        best_pos = int(_np.nanargmin(df["plot_value"].to_numpy(float)))
        best_trial_num = int(df.iloc[best_pos]["trial_number"])
        ...
        sm = _plt.cm.ScalarMappable(norm=norm, cmap=cmap)
        sm.set_array([])
        ...
        _plt.setp(leg.get_lines(), linewidth=5.0)
        leg.set_zorder(20)

        fig.subplots_adjust(bottom=0.24)
        fig.tight_layout()

        pathp = os.path.join(out_dir, f"{stem}_parallel_coordinates.{replot_args.plot_file_format}")
        fig.savefig(pathp, dpi=int(replot_args.plot_dpi), format=replot_args.plot_file_format)
        _plt.close(fig)
        made.append(pathp)

    # ------------------------------------------------------------------
    # 3D landscapes and contour maps
    # ------------------------------------------------------------------
    for x_name, y_name in param_pairs:
        if x_name not in df.columns or y_name not in df.columns:
            print(f"[HPO Replot] Skipping pair {x_name},{y_name}; columns not found in CSV")
            continue

        pair_df = df.copy()
        pair_df["_x_plot"], x_label = _plot_vals(pair_df[x_name], x_name, for_parallel=False)
        pair_df["_y_plot"], y_label = _plot_vals(pair_df[y_name], y_name, for_parallel=False)

        surf = pair_df.groupby(["_x_plot", "_y_plot"], as_index=False).agg(
            plot_value=("plot_value", "mean"),
            trial_number=("trial_number", "min"),
        )

        tri = None
        if len(surf) >= 3 and len(_np.unique(surf["_x_plot"])) >= 2 and len(_np.unique(surf["_y_plot"])) >= 2:
            try:
                tri = _mtri.Triangulation(
                    surf["_x_plot"].to_numpy(float),
                    surf["_y_plot"].to_numpy(float),
                )
            except Exception:
                tri = None

        if tri is None:
            print(f"[HPO Replot] Skipping pair {x_name},{y_name}; insufficient unique coordinates")
            continue

        best = pair_df.loc[int(pair_df["plot_value"].idxmin())]

        interp = None
        try:
            interp = _mtri.LinearTriInterpolator(tri, surf["plot_value"].to_numpy(float))
        except Exception:
            interp = None
        ...
        if interp is not None:
            grid_size = max(50, int(replot_args.hpo_landscape_grid_size))
            xi = _np.linspace(float(surf["_x_plot"].min()), float(surf["_x_plot"].max()), grid_size)
            yi = _np.linspace(float(surf["_y_plot"].min()), float(surf["_y_plot"].max()), grid_size)
            Xi, Yi = _np.meshgrid(xi, yi)
            Zi = interp(Xi, Yi)
            Zi = _np.asarray(_np.ma.filled(Zi, _np.nan), dtype=float)

        fig = _plt.figure(figsize=(10, 7))
        ax = fig.add_subplot(111, projection="3d")

        if Zi is not None and _np.isfinite(Zi).any():
            ...
            surf_obj = ax.plot_trisurf(
                tri,
                surf["plot_value"].to_numpy(float),
                cmap="turbo",
                alpha=0.84,
                linewidth=0.08,
                antialiased=True,
        ...
        fig.colorbar(surf_obj, ax=ax, shrink=0.62, pad=0.08).set_label(metric_label)

        pts = ax.scatter(
            pair_df["_x_plot"],
            pair_df["_y_plot"],
            pair_df["plot_value"],
            c=pair_df["trial_number"],
            cmap="plasma",
            s=42,
            edgecolor="k",
            linewidth=0.25,
        ...
        fig.colorbar(pts, ax=ax, shrink=0.62, pad=0.02).set_label("Trial number")
        ...
        ax.set_title(f"HPO Loss Landscape: {x_name} vs {y_name}")
        ax.view_init(elev=28, azim=-135)
        fig.tight_layout()

        path3d = os.path.join(
            out_dir,
            f"{stem}_3d_landscape_{x_name}_vs_{y_name}.{replot_args.plot_file_format}",
        )

        fig.savefig(path3d, dpi=int(replot_args.plot_dpi), format=replot_args.plot_file_format)
        _plt.close(fig)
        made.append(path3d)
        ...
        if Zi is not None and _np.isfinite(Zi).any():
            cf = ax.contourf(Xi, Yi, Zi, levels=40, cmap="magma")
            ax.contour(Xi, Yi, Zi, levels=16, colors="white", linewidths=0.35, alpha=0.45)
        else:
            cf = ax.tricontourf(tri, surf["plot_value"].to_numpy(float), levels=28, cmap="magma")
            ax.tricontour(tri, surf["plot_value"].to_numpy(float), levels=12, colors="white", linewidths=0.35, alpha=0.45)

        fig.colorbar(cf, ax=ax).set_label(metric_label)

        n = max(len(pair_df), 1)
        alphas = 0.18 + 0.82 * (_np.arange(n) / max(n - 1, 1))

        norm = _plt.Normalize(pair_df["trial_number"].min(), pair_df["trial_number"].max())
        colors = _plt.get_cmap("plasma")(norm(pair_df["trial_number"].to_numpy(float)))
        colors[:, 3] = alphas
        ...
        ax.set_xlabel(x_label)
        ax.set_ylabel(y_label)
        ax.set_title(f"HPO Topographic Survey: {x_name} vs {y_name}")
        ...
        path2d = os.path.join(
            out_dir,
            f"{stem}_contour_{x_name}_vs_{y_name}.{replot_args.plot_file_format}",
        )

        fig.savefig(path2d, dpi=int(replot_args.plot_dpi), format=replot_args.plot_file_format)
        _plt.close(fig)
        made.append(path2d)

    print("=" * 88)
    print("HPO Replot From CSV")
    print("=" * 88)
...
def _maybe_run_fast_hpo_visualize_demo_from_cli() -> None:
    """Generate demo HPO plots before importing torch/xgboost/dataset code.

    This keeps `--hpo-visualize-demo` genuinely fast on clusters: it only
    imports numpy/pandas/matplotlib and writes synthetic example plots.
    """
    if "--hpo-visualize-demo" not in sys.argv:
        return

    demo_parser = argparse.ArgumentParser(add_help=True)
    demo_parser.add_argument("--hpo-visualize-demo", action="store_true")
    demo_parser.add_argument("--logdir", default="runs/peat_xgb")
    demo_parser.add_argument("--hpo-demo-trials", type=int, default=60)
    demo_parser.add_argument("--hpo-demo-seed", type=int, default=12345)
    demo_parser.add_argument("--hpo-visualize-params", type=str, default="eta,max_depth")
    demo_parser.add_argument("--hpo-visualize-metric", type=str, default="value")
    demo_parser.add_argument("--hpo-objective-metric", type=str, default="logloss")
    demo_parser.add_argument("--plot-file-format", type=str, default="png")
    demo_parser.add_argument("--plot-dpi", type=int, default=850)
    demo_parser.add_argument("--hpo-landscape-grid-size", type=int, default=160)
    ...
    out_dir = os.path.join(demo_args.logdir, "hpo_visualizations")
    os.makedirs(out_dir, exist_ok=True)

    rng = _np.random.default_rng(int(demo_args.hpo_demo_seed))
    n_trials = max(8, int(demo_args.hpo_demo_trials))
    rows = []
    log_eta_lo, log_eta_hi = _np.log10(0.0008), _np.log10(0.12)
    log_eta_best = _np.log10(0.018)
    depth_best = 6.0

    for i in range(n_trials):
        progress = i / max(n_trials - 1, 1)
        if progress < 0.38:
            log_eta = rng.uniform(log_eta_lo, log_eta_hi)
            max_depth = int(rng.integers(3, 13))
            min_child_weight = 10 ** rng.uniform(_np.log10(0.3), _np.log10(30.0))
            subsample = rng.uniform(0.55, 1.0)
            colsample_bynode = rng.uniform(0.35, 1.0)
            gamma = rng.uniform(0.0, 2.0)
            noise = rng.normal(0.0, 0.00085)
        else:
            spread = max(0.06, 0.36 * (1.0 - progress))
            log_eta = _np.clip(rng.normal(log_eta_best, spread), log_eta_lo, log_eta_hi)
            max_depth = int(_np.clip(_np.round(rng.normal(depth_best, max(0.75, 2.2 * (1.0 - progress)))), 3, 12))
            min_child_weight = float(_np.clip(10 ** rng.normal(_np.log10(2.5), max(0.12, 0.45 * (1.0 - progress))), 0.3, 30.0))
            subsample = float(_np.clip(rng.normal(0.88, max(0.025, 0.14 * (1.0 - progress))), 0.55, 1.0))
            colsample_bynode = float(_np.clip(rng.normal(0.62, max(0.025, 0.17 * (1.0 - progress))), 0.35, 1.0))
            gamma = float(_np.clip(rng.normal(0.08, max(0.02, 0.28 * (1.0 - progress))), 0.0, 2.0))
            noise = rng.normal(0.0, 0.00045)

        eta = float(10 ** log_eta)
        num_round = int(_np.clip(_np.round(170 - 34 * _np.log10(eta / 0.001)), 35, 240))
        value = float(max(0.001,
            0.021
            + 0.014 * ((log_eta - log_eta_best) / 0.55) ** 2
            + 0.0026 * ((max_depth - depth_best) / 1.65) ** 2
            + 0.010 * max(0.0, max_depth - 8.0) ** 1.25
            + 0.0045 * (subsample - 0.88) ** 2
            + 0.0035 * (colsample_bynode - 0.62) ** 2
            + 0.0015 * gamma
            + noise
        ))
        rows.append({
            "trial_number": i, "value": value, "brier_score": value,
            "log_loss": float(value * 2.4 + 0.015 + rng.normal(0.0, 0.001)),
            "ece": float(max(0.0005, 0.004 + value * 0.18 + rng.normal(0.0, 0.00035))),
            "eta": eta, "max_depth": max_depth, "min_child_weight": min_child_weight,
            "subsample": subsample, "colsample_bynode": colsample_bynode, "gamma": gamma,
            "num_round": num_round, "best_iteration": int(_np.clip(num_round * rng.uniform(0.45, 0.92), 10, num_round)),
        })

    df = _pd.DataFrame(rows)
    metric = demo_args.hpo_visualize_metric if demo_args.hpo_visualize_metric in df.columns else "value"
    df["plot_value"] = df[metric]
    params = [x.strip() for x in demo_args.hpo_visualize_params.split(",") if x.strip()]
    x_name = params[0] if len(params) >= 1 and params[0] in df.columns else "eta"
    y_name = params[1] if len(params) >= 2 and params[1] in df.columns else "max_depth"

    def _metric_label(name):
        key = str(name)
        if key == "value":
            aliases = {"brier": "brier_score", "logloss": "log_loss"}
            key = aliases.get(str(demo_args.hpo_objective_metric).strip().lower(), str(demo_args.hpo_objective_metric))
        labels = {
            "value": "Validation log loss (lower is better)",
            "brier_score": "Validation Brier score (lower is better)",
            "brier": "Validation Brier score (lower is better)",
    ...
    df["_x_plot"], x_label = _plot_vals(df[x_name], x_name, for_parallel=False)
    df["_y_plot"], y_label = _plot_vals(df[y_name], y_name, for_parallel=False)
    surf = df.groupby(["_x_plot", "_y_plot"], as_index=False).agg(plot_value=("plot_value", "mean"), trial_number=("trial_number", "min"))
    csv_path = os.path.join(out_dir, "hpo_demo_plot_trials.csv")
    df.to_csv(csv_path, index=False)

    tri = None
    if len(surf) >= 3 and len(_np.unique(surf["_x_plot"])) >= 2 and len(_np.unique(surf["_y_plot"])) >= 2:
        try:
            tri = _mtri.Triangulation(surf["_x_plot"].to_numpy(float), surf["_y_plot"].to_numpy(float))
        except Exception:
            tri = None

    made = [csv_path]

    # Generate the parallel-coordinate overview first, then the pairwise maps.
    pnames = [p for p in "eta,max_depth,min_child_weight,subsample,colsample_bynode,gamma,num_round".split(",") if p in df.columns]
    normed, labels = [], []
    for name in pnames:
        vals, label = _plot_vals(df[name], name, for_parallel=True)
        mn, mx = float(_np.nanmin(vals)), float(_np.nanmax(vals))
    ...
    if len(normed) >= 2:
        ...
        for i in range(Y.shape[0]):
            ax.plot(xs, Y[i], color=cmap(norm(df.iloc[i]["trial_number"])), alpha=0.18 + 0.82 * (i / max(Y.shape[0] - 1, 1)), linewidth=1.1)
        best_pos = int(_np.nanargmin(df["plot_value"].to_numpy(float)))
        ax.plot(xs, Y[best_pos], color="black", linewidth=3.0, alpha=0.9, label=f"best trial #{int(df.iloc[best_pos]['trial_number'])}")
        sm = _plt.cm.ScalarMappable(norm=norm, cmap=cmap); sm.set_array([])
        fig.colorbar(sm, ax=ax, pad=0.02).set_label("Trial number")
        ...
        fig.subplots_adjust(bottom=0.24)
        fig.tight_layout()
        pathp = os.path.join(out_dir, f"hpo_demo_parallel_coordinates.{demo_args.plot_file_format}")
        fig.savefig(pathp, dpi=int(demo_args.plot_dpi), format=demo_args.plot_file_format)
        _plt.close(fig)
        made.append(pathp)


    if tri is not None:
        best = df.loc[int(df["plot_value"].idxmin())]
        ...
        Xi = Yi = Zi = None
        if interp is not None:
            grid_size = max(50, int(demo_args.hpo_landscape_grid_size))
            xi = _np.linspace(float(surf["_x_plot"].min()), float(surf["_x_plot"].max()), grid_size)
            yi = _np.linspace(float(surf["_y_plot"].min()), float(surf["_y_plot"].max()), grid_size)
            Xi, Yi = _np.meshgrid(xi, yi)
            Zi = interp(Xi, Yi)
        ...
        ax = fig.add_subplot(111, projection="3d")
        if Zi is not None and _np.isfinite(Zi).any():
            surf_obj = ax.plot_surface(Xi, Yi, Zi, cmap="turbo", alpha=0.88, linewidth=0.0, antialiased=True)
        else:
            surf_obj = ax.plot_trisurf(tri, surf["plot_value"].to_numpy(float), cmap="turbo", alpha=0.84, linewidth=0.08, antialiased=True)
        fig.colorbar(surf_obj, ax=ax, shrink=0.62, pad=0.08).set_label(metric_label)
        pts = ax.scatter(df["_x_plot"], df["_y_plot"], df["plot_value"], c=df["trial_number"], cmap="plasma", s=42, edgecolor="k", linewidth=0.25)
        fig.colorbar(pts, ax=ax, shrink=0.62, pad=0.02).set_label("Trial number")
        ax.scatter([best["_x_plot"]], [best["_y_plot"]], [best["plot_value"]], marker="*", s=260, c="white", edgecolor="black", linewidth=1.2)
        ax.text(best["_x_plot"], best["_y_plot"], best["plot_value"], f"  best #{int(best['trial_number'])}", fontsize=9)
        ax.set_xlabel(x_label); ax.set_ylabel(y_label); ax.set_zlabel(metric_label)
        ax.set_title(f"Demo HPO Loss Landscape: {x_name} vs {y_name}")
        ax.view_init(elev=28, azim=-135)
        fig.tight_layout()
        path3d = os.path.join(out_dir, f"hpo_demo_3d_landscape_{x_name}_vs_{y_name}.{demo_args.plot_file_format}")
        fig.savefig(path3d, dpi=int(demo_args.plot_dpi), format=demo_args.plot_file_format)
        _plt.close(fig)
        made.append(path3d)
        ...
        alphas = 0.18 + 0.82 * (_np.arange(n) / max(n - 1, 1))
        norm = _plt.Normalize(df["trial_number"].min(), df["trial_number"].max())
        colors = _plt.get_cmap("plasma")(norm(df["trial_number"].to_numpy(float)))
        colors[:, 3] = alphas
        ax.scatter(df["_x_plot"], df["_y_plot"], s=44, c=colors, edgecolor="white", linewidth=0.35)
        ax.scatter([best["_x_plot"]], [best["_y_plot"]], marker="*", s=280, c="cyan", edgecolor="black", linewidth=1.0, label=f"best trial #{int(best['trial_number'])}")
        ax.set_xlabel(x_label); ax.set_ylabel(y_label)
        ax.set_title(f"Demo HPO Topographic Survey: {x_name} vs {y_name}")
        ax.grid(True, linestyle="--", alpha=0.22); ax.legend(loc="best")
        fig.tight_layout()
        path2d = os.path.join(out_dir, f"hpo_demo_contour_{x_name}_vs_{y_name}.{demo_args.plot_file_format}")
        fig.savefig(path2d, dpi=int(demo_args.plot_dpi), format=demo_args.plot_file_format)
        _plt.close(fig)
        made.append(path2d)

    print("=" * 88)
    print("HPO Visualization Quick Demo")
    print("=" * 88)
    for path in made:
        print(f"  • {path}")
    raise SystemExit(0)
...
_maybe_run_fast_hpo_replot_from_csv_from_cli()
_maybe_run_fast_hpo_visualize_demo_from_cli()
...
try:
    ...
    _HAVE_DATASET_BUILDER = True
except Exception as e:
    JointPeatDataset = None  # type: ignore
    parse_input_spec = None  # type: ignore
    _HAVE_DATASET_BUILDER = False
    print(f"DATASET BUILDER NOT IMPORTED ({e}); --hpo-visualize-demo still works, but training requires joint_peat_dataset_builder_cluster.py")
...
def status(tag: str, msg: str):
    """Timestamped progress print for long-running jobs."""
    try:
...
def status_kv(tag: str, **items):
    """Compact key=value progress print."""
    msg = " | ".join(f"{k}={v}" for k, v in items.items())
    status(tag, msg)


def _shape_str(x) -> str:
    try:
        return "x".join(str(int(v)) for v in x.shape)
    except Exception:
        return "unknown"
...
def _hpo_trial_primary_value(trial) -> Optional[float]:
    """Return the scalar objective used for landscape plots.

    For single-objective studies this is trial.value. For multi-objective
    NSGA-II studies this uses the first objective, which this script treats
    as the Brier/objective anchor when selecting a Pareto trial.
    """
    val = None
    try:
        val = trial.value
...
def _hpo_trial_is_complete(trial) -> bool:
    """Return True for completed Optuna trials, or demo trials with COMPLETE state."""
    state = getattr(trial, "state", None)
    if _HAVE_OPTUNA:
        try:
            return state == optuna.trial.TrialState.COMPLETE
        except Exception:
            pass
    state_name = getattr(state, "name", None)
    if state_name is not None:
        return str(state_name).upper() == "COMPLETE"
    return str(state).upper() == "COMPLETE"


def _hpo_completed_trials_frame(study, metric_name: str = "value") -> pd.DataFrame:
    """Flatten completed Optuna/demo trials into one row per trial for plotting."""
    if study is None:
        return pd.DataFrame()

    rows = []
    for trial in getattr(study, "trials", []):
        if not _hpo_trial_is_complete(trial):
            continue

        primary_value = _hpo_trial_primary_value(trial)
        ...
        # Flatten numeric params.
        for k, v in (trial.params or {}).items():
            try:
                row[k] = float(v)
            except Exception:
                row[k] = v

        # Pull numeric metrics collected inside hpo_objective.
        metrics = (trial.user_attrs or {}).get("metrics", {}) or {}
        if isinstance(metrics, dict):
            for k, v in metrics.items():
                if isinstance(v, (int, float, np.integer, np.floating)):
                    row[k] = float(v)

        # Best iteration is useful in the parallel coordinate plot.
        if "best_iteration" in (trial.user_attrs or {}):
            try:
                row["best_iteration"] = float(trial.user_attrs["best_iteration"])
            except Exception:
                pass

        if metric_name != "value" and metric_name in row:
            row["plot_value"] = float(row[metric_name])
        else:
            row["plot_value"] = float(row["value"])
...
def _split_csv_names(spec: str) -> List[str]:
    return [x.strip() for x in str(spec or "").split(",") if x.strip()]


def _parse_hpo_param_pairs(spec: str) -> List[Tuple[str, str]]:
    """Parse one or more 2D HPO parameter pairs.

    Accepts a single pair like "eta,max_depth" or multiple pairs separated
    by semicolons, for example:
    "eta,max_depth;eta,min_child_weight;subsample,colsample_bynode".
    """
    chunks = [c.strip() for c in str(spec or "").split(";") if c.strip()]
    if not chunks:
        return [("eta", "max_depth")]
    ...
    for chunk in chunks:
        names = _split_csv_names(chunk)
        if len(names) >= 2:
            pairs.append((names[0], names[1]))
    return pairs or [("eta", "max_depth")]


def _normalize_hpo_objective_metric(metric_name: str) -> str:
    """Normalize CLI aliases to the trial_metrics key used as Optuna's scalar objective."""
    aliases = {
        "brier": "brier_score",
    ...
    return aliases.get(str(metric_name or "brier_score").strip().lower(), str(metric_name or "brier_score").strip())


def _hpo_metric_display_label(metric_name: str, args=None) -> str:
    metric_key = str(metric_name)
    if metric_key == "value" and args is not None:
        metric_key = _normalize_hpo_objective_metric(getattr(args, "hpo_objective_metric", "logloss"))
    labels = {
        "value": "Validation log loss (lower is better)",
        "brier_score": "Validation Brier score (lower is better)",
        "brier": "Validation Brier score (lower is better)",
        ...
        "ece": "Validation expected calibration error / ECE (lower is better)",
        "mce": "Validation maximum calibration error / MCE (lower is better)",
        "rmse": "Validation RMSE (lower is better)",
    }
    return labels.get(str(metric_key), f"{metric_key} (lower is better)")
...
def _hpo_landscape_frames(df: pd.DataFrame, x_name: str, y_name: str) -> Tuple[pd.DataFrame, pd.DataFrame, str, str]:
    cols = ["trial_number", "plot_value", x_name, y_name]
    raw = df[cols].copy()
    for c in cols:
        raw[c] = pd.to_numeric(raw[c], errors="coerce")
    raw = raw.dropna(subset=["plot_value", x_name, y_name])
    if raw.empty:
        return raw, raw, x_name, y_name

    raw["_x_plot"], x_label = _hpo_plot_transform(raw[x_name], x_name)
    raw["_y_plot"], y_label = _hpo_plot_transform(raw[y_name], y_name)

    # Average duplicate sampled coordinates so triangulation gets unique x/y pairs.
    surf = (
        raw.groupby(["_x_plot", "_y_plot"], as_index=False)
        .agg(plot_value=("plot_value", "mean"), trial_number=("trial_number", "min"))
        .sort_values("trial_number")
        .reset_index(drop=True)
    )
    return raw, surf, x_label, y_label


def _safe_make_triangulation(x: np.ndarray, y: np.ndarray):
    ...
    try:
        ...
        print(f"[HPO Viz] Triangulation skipped: {e}")
        return None


def _safe_color_norm(values):
    """Matplotlib Normalize that survives single-value demo/test cases."""
    import matplotlib.pyplot as plt
    vals = np.asarray(values, dtype=float)
    finite = vals[np.isfinite(vals)]
    if finite.size == 0:
        return plt.Normalize(0.0, 1.0)
...
def _interpolate_hpo_surface(surf: pd.DataFrame, grid_size: int = 90):
    """Interpolate sparse Optuna trials to a display grid for Plotly contours."""
    import matplotlib.tri as mtri
    x = surf["_x_plot"].to_numpy(dtype=float)
    y = surf["_y_plot"].to_numpy(dtype=float)
    z = surf["plot_value"].to_numpy(dtype=float)
    tri = _safe_make_triangulation(x, y)
    if tri is None:
        return None
    try:
        ...
        print(f"[HPO Viz] Surface interpolation skipped: {e}")
        return None


def plot_hpo_3d_landscape(study, args, x_name: str = "eta", y_name: str = "max_depth", metric_name: str = "value") -> Optional[str]:
    """Save a presentation-ready 3D HPO loss landscape.

    Lower z-values are better. The surface is an interpolation of sampled
    Optuna trials, while the points are the actual evaluated trials.
    """
    import matplotlib.pyplot as plt

    df = _hpo_completed_trials_frame(study, metric_name=metric_name)
    if df.empty or x_name not in df.columns or y_name not in df.columns:
        print(f"[HPO Viz] Skipping 3D landscape; missing completed trials or params: {x_name}, {y_name}")
        return None

    raw, surf, x_label, y_label = _hpo_landscape_frames(df, x_name, y_name)
    if len(surf) < 3:
        print("[HPO Viz] Skipping 3D landscape; need at least 3 unique trial coordinates")
        return None

    out_dir = os.path.join(args.logdir, "hpo_visualizations")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"hpo_3d_landscape_{x_name}_vs_{y_name}.{args.plot_file_format}")

    x = surf["_x_plot"].to_numpy(dtype=float)
    y = surf["_y_plot"].to_numpy(dtype=float)
    z = surf["plot_value"].to_numpy(dtype=float)
    tri = _safe_make_triangulation(x, y)
    metric_label = _hpo_metric_display_label(metric_name, args)
    interp_surface = _interpolate_hpo_surface(surf, grid_size=int(getattr(args, "hpo_landscape_grid_size", 160)))

    fig = plt.figure(figsize=(10, 7))
    ax = fig.add_subplot(111, projection="3d")

    if interp_surface is not None:
        xi, yi, Zi = interp_surface
        Xi, Yi = np.meshgrid(xi, yi)
        Zi = np.asarray(np.ma.filled(Zi, np.nan), dtype=float)
        surf_obj = ax.plot_surface(Xi, Yi, Zi, cmap="turbo", alpha=0.88, linewidth=0.0, antialiased=True)
        cbar = fig.colorbar(surf_obj, ax=ax, shrink=0.62, pad=0.08)
        cbar.set_label(metric_label)
    elif tri is not None:
        surf_obj = ax.plot_trisurf(tri, z, cmap="turbo", alpha=0.82, linewidth=0.08, antialiased=True)
        cbar = fig.colorbar(surf_obj, ax=ax, shrink=0.62, pad=0.08)
        cbar.set_label(metric_label)

    points = ax.scatter(
        raw["_x_plot"], raw["_y_plot"], raw["plot_value"],
        c=raw["trial_number"], cmap="plasma", s=42, edgecolor="k", linewidth=0.25,
        depthshade=True,
    )
    pbar = fig.colorbar(points, ax=ax, shrink=0.62, pad=0.02)
    pbar.set_label("Trial number")

    best_i = int(raw["plot_value"].idxmin())
    best = raw.loc[best_i]
    ax.scatter([best["_x_plot"]], [best["_y_plot"]], [best["plot_value"]], marker="*", s=260, c="white", edgecolor="black", linewidth=1.2)
    ax.text(best["_x_plot"], best["_y_plot"], best["plot_value"], f"  best #{int(best['trial_number'])}", fontsize=9)
    ...
    ax.view_init(elev=28, azim=-135)
    fig.tight_layout()
    fig.savefig(out_path, dpi=int(getattr(args, "plot_dpi", 150)), format=args.plot_file_format)
    plt.close(fig)
    print(f"[HPO Viz] Saved 3D landscape: {out_path}")
    return out_path


def plot_hpo_contour_map(study, args, x_name: str = "eta", y_name: str = "max_depth", metric_name: str = "value") -> Optional[str]:
    """Save a 2D topographic contour map with trial evolution overlay."""
    import matplotlib.pyplot as plt

    df = _hpo_completed_trials_frame(study, metric_name=metric_name)
    if df.empty or x_name not in df.columns or y_name not in df.columns:
        print(f"[HPO Viz] Skipping contour map; missing completed trials or params: {x_name}, {y_name}")
        return None

    raw, surf, x_label, y_label = _hpo_landscape_frames(df, x_name, y_name)
    if len(surf) < 3:
        print("[HPO Viz] Skipping contour map; need at least 3 unique trial coordinates")
        return None

    out_dir = os.path.join(args.logdir, "hpo_visualizations")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"hpo_contour_{x_name}_vs_{y_name}.{args.plot_file_format}")
    ...
    if interp_surface is not None:
        ...
        cf = ax.tricontourf(tri, z, levels=28, cmap="magma")
        ax.tricontour(tri, z, levels=12, colors="white", linewidths=0.35, alpha=0.45)
        cbar = fig.colorbar(cf, ax=ax)
        cbar.set_label(metric_label)

    # Fade-in effect by sequence: early trials are faint, late trials are solid.
    n = max(len(raw), 1)
    alphas = 0.18 + 0.82 * (np.arange(n) / max(n - 1, 1))
    norm = _safe_color_norm(raw["trial_number"].to_numpy(dtype=float))
    cmap = plt.get_cmap("plasma")
    colors = cmap(norm(raw["trial_number"].to_numpy(dtype=float)))
    colors[:, 3] = alphas

    ax.scatter(raw["_x_plot"], raw["_y_plot"], s=44, c=colors, edgecolor="white", linewidth=0.35)
    ...
    fig.savefig(out_path, dpi=int(getattr(args, "plot_dpi", 150)), format=args.plot_file_format)
    plt.close(fig)
    print(f"[HPO Viz] Saved contour map: {out_path}")
    return out_path


def plot_hpo_parallel_coordinates(study, args, param_names: Optional[List[str]] = None, metric_name: str = "value") -> Optional[str]:
    """Save a parallel-coordinate plot across many HPO parameters."""
    import matplotlib.pyplot as plt

    df = _hpo_completed_trials_frame(study, metric_name=metric_name)
    if df.empty:
        print("[HPO Viz] Skipping parallel coordinates; no completed trials")
        return None

    if param_names is None:
        param_names = _split_csv_names(getattr(args, "hpo_parallel_params", ""))
    param_names = [p for p in param_names if p in df.columns]
    if len(param_names) < 2:
        print("[HPO Viz] Skipping parallel coordinates; need at least two available params")
        return None

    out_dir = os.path.join(args.logdir, "hpo_visualizations")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"hpo_parallel_coordinates.{args.plot_file_format}")

    pdata = df[["trial_number", "plot_value"] + param_names].copy()
    for c in ["trial_number", "plot_value"] + param_names:
        pdata[c] = pd.to_numeric(pdata[c], errors="coerce")
    pdata = pdata.dropna().sort_values("trial_number")
    if pdata.empty:
        print("[HPO Viz] Skipping parallel coordinates; no numeric complete rows")
        return None

    labels = []
    normed_cols = []
    for p_name in param_names:
        vals, label = _hpo_plot_transform(pdata[p_name], p_name, for_parallel=True)
        mn, mx = float(np.nanmin(vals)), float(np.nanmax(vals))
        if not np.isfinite(mn) or not np.isfinite(mx) or mx <= mn:
            continue
        normed_cols.append((vals - mn) / (mx - mn))
        labels.append(f"{label}\n[{mn:.3g}, {mx:.3g}]")

    if len(normed_cols) < 2:
        print("[HPO Viz] Skipping parallel coordinates; parameters have insufficient variation")
        return None

    Y = np.vstack(normed_cols).T
    xs = np.arange(Y.shape[1])

    fig, ax = plt.subplots(figsize=(max(11.5, 1.9 * Y.shape[1]), 7.2))

    color_by = getattr(args, "hpo_visualize_color_by", "value")
    color_is_trial = str(color_by) in ("trial", "trial_number")
    color_values = pdata["trial_number"].to_numpy(dtype=float) if color_is_trial else pdata["plot_value"].to_numpy(dtype=float)

    cmap_name = "plasma" if color_is_trial else "viridis_r"
    norm = _safe_color_norm(color_values)
    ...
    best_pos = int(np.nanargmin(pdata["plot_value"].to_numpy(dtype=float)))
    best_trial_num = int(pdata.iloc[best_pos]["trial_number"])
    ...
    leg.get_frame().set_edgecolor("black")
    leg.get_frame().set_linewidth(1.5)
    plt.setp(leg.get_lines(), linewidth=5.0)

    fig.tight_layout()
    fig.savefig(out_path, dpi=int(getattr(args, "plot_dpi", 150)), format=args.plot_file_format)
    plt.close(fig)

    print(f"[HPO Viz] Saved parallel coordinates: {out_path}")
    return out_path


def save_hpo_interactive_html(study, args, x_name: str = "eta", y_name: str = "max_depth", metric_name: str = "value") -> List[str]:
    """Save optional Plotly HTML versions when Plotly is installed."""
    out_paths: List[str] = []
    try:
        import plotly.graph_objects as go
    except Exception as e:
        print(f"[HPO Viz] Plotly not installed; skipping interactive HTML ({e})")
        return out_paths
    ...
    # 3D interactive mesh + actual trials.
    html3d = os.path.join(out_dir, f"hpo_3d_landscape_{x_name}_vs_{y_name}.html")
    fig3d = go.Figure()
    fig3d.add_trace(go.Mesh3d(
        x=surf["_x_plot"], y=surf["_y_plot"], z=surf["plot_value"],
        intensity=surf["plot_value"], colorscale="Turbo", opacity=0.62,
        name="interpolated trial surface", showscale=True,
        colorbar=dict(title=metric_name),
    ))
    fig3d.add_trace(go.Scatter3d(
        x=raw["_x_plot"], y=raw["_y_plot"], z=raw["plot_value"],
        mode="markers",
        marker=dict(size=5, color=raw["trial_number"], colorscale="Plasma", showscale=True, colorbar=dict(title="Trial #", x=1.08)),
        text=[f"trial #{int(t)}<br>{metric_name}={v:.6g}" for t, v in zip(raw["trial_number"], raw["plot_value"])],
        name="actual trials",
    ))
    fig3d.update_layout(
        title=f"Interactive HPO Loss Landscape: {x_name} vs {y_name}",
        scene=dict(xaxis_title=x_label, yaxis_title=y_label, zaxis_title=metric_name),
        margin=dict(l=0, r=0, b=0, t=45),
    )
    fig3d.write_html(html3d)
    print(f"[HPO Viz] Saved interactive 3D HTML: {html3d}")
    out_paths.append(html3d)

    # 2D interactive contour, when interpolation succeeds.
    interp = _interpolate_hpo_surface(surf)
    if interp is not None:
        xi, yi, Zi = interp
        zi = np.asarray(Zi.filled(np.nan) if hasattr(Zi, "filled") else Zi, dtype=float)
        html2d = os.path.join(out_dir, f"hpo_interactive_contour_{x_name}_vs_{y_name}.html")
        fig2d = go.Figure()
        fig2d.add_trace(go.Contour(
            x=xi, y=yi, z=zi,
            colorscale="Magma", contours=dict(showlabels=True),
            colorbar=dict(title=_hpo_metric_display_label(metric_name, args)), name="interpolated contour",
        ))
        fig2d.add_trace(go.Scatter(
            x=raw["_x_plot"], y=raw["_y_plot"], mode="markers",
            marker=dict(size=8, color=raw["trial_number"], colorscale="Plasma", showscale=True, colorbar=dict(title="Trial #", x=1.08), line=dict(width=0.5, color="white")),
            text=[f"trial #{int(t)}<br>{metric_name}={v:.6g}" for t, v in zip(raw["trial_number"], raw["plot_value"])],
            name="actual trials",
        ))
        fig2d.update_layout(
            title=f"Interactive HPO Topographic Survey: {x_name} vs {y_name}",
            xaxis_title=x_label,
            yaxis_title=y_label,
            margin=dict(l=50, r=90, b=50, t=50),
        )
        fig2d.write_html(html2d)
        print(f"[HPO Viz] Saved interactive contour HTML: {html2d}")
        out_paths.append(html2d)

    return out_paths


def save_hpo_visualizations(study, args, prefix: str = "hpo") -> List[str]:
    """Create all requested HPO visualization assets after study.optimize()."""
    if not getattr(args, "hpo_visualize", False):
        return []

    metric_name = getattr(args, "hpo_visualize_metric", "value")
    param_pairs = _parse_hpo_param_pairs(getattr(args, "hpo_visualize_params", "eta,max_depth"))
    ...
    df = _hpo_completed_trials_frame(study, metric_name=metric_name)
    if df.empty:
        print("[HPO Viz] No completed HPO trials to visualize")
        return []
    csv_path = os.path.join(out_dir, f"{prefix}_plot_trials.csv")
    df.to_csv(csv_path, index=False)
    print(f"[HPO Viz] Saved flattened plot data: {csv_path}")

    metadata = {
        "plot_trials_csv": csv_path,
        "metric_name": metric_name,
        "metric_label": _hpo_metric_display_label(metric_name, args),
        "hpo_objective_metric": getattr(args, "hpo_objective_metric", None),
        "hpo_visualize_param_pairs": [[x, y] for (x, y) in param_pairs],
        "hpo_parallel_params": _split_csv_names(getattr(args, "hpo_parallel_params", "")),
        "plot_file_format": getattr(args, "plot_file_format", "png"),
        "plot_dpi": int(getattr(args, "plot_dpi", 150)),
        "hpo_landscape_grid_size": int(getattr(args, "hpo_landscape_grid_size", 160)),
    }
    metadata_path = os.path.join(out_dir, f"{prefix}_plot_metadata.json")
    with open(metadata_path, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=2)
    print(f"[HPO Viz] Saved plot metadata: {metadata_path}")

    made: List[str] = [csv_path, metadata_path]
    status("HPO Viz", "plot order: parallel coordinates first, then 3D landscapes and contour maps")

    # Generate the parallel-coordinate plot first so the broad multi-parameter
    # overview is available before the pairwise landscape/contour plots.
    ppath = plot_hpo_parallel_coordinates(study, args, metric_name=metric_name)
    if ppath:
        made.append(ppath)

    for x_name, y_name in param_pairs:
        for path in [
            plot_hpo_3d_landscape(study, args, x_name=x_name, y_name=y_name, metric_name=metric_name),
            plot_hpo_contour_map(study, args, x_name=x_name, y_name=y_name, metric_name=metric_name),
        ]:
            if path:
                made.append(path)
        made.extend(save_hpo_interactive_html(study, args, x_name=x_name, y_name=y_name, metric_name=metric_name))
...
@dataclass
class _DemoHPOTrial:
    """Tiny Optuna-like trial used by --hpo-visualize-demo."""
    number: int
    value: float
    params: Dict[str, Any]
...
@dataclass
class _DemoHPOStudy:
    """Tiny Optuna-like study used by --hpo-visualize-demo."""
    trials: List[_DemoHPOTrial]


def make_demo_hpo_study(n_trials: int = 60, seed: int = 12345) -> _DemoHPOStudy:
    """Create a fast synthetic HPO trajectory for visual-demo plots.

    This does not train a model. It creates Optuna-shaped trial records where
    early trials explore broadly and later trials concentrate near a known
    low-error basin, mimicking TPE convergence for presentation checks.
    """
    rng = np.random.default_rng(int(seed))
    n_trials = max(8, int(n_trials))

    trials: List[_DemoHPOTrial] = []
    log_eta_lo, log_eta_hi = np.log10(0.0008), np.log10(0.12)
    log_eta_best = np.log10(0.018)
    depth_best = 6.0

    for i in range(n_trials):
        progress = i / max(n_trials - 1, 1)
        explore = progress < 0.38

        if explore:
            log_eta = rng.uniform(log_eta_lo, log_eta_hi)
            max_depth = int(rng.integers(3, 13))
            min_child_weight = 10 ** rng.uniform(np.log10(0.3), np.log10(30.0))
            subsample = rng.uniform(0.55, 1.0)
            colsample_bynode = rng.uniform(0.35, 1.0)
            gamma = rng.uniform(0.0, 2.0)
        else:
            # Later trials increasingly cluster into the valley.
            spread = max(0.06, 0.36 * (1.0 - progress))
            log_eta = np.clip(rng.normal(log_eta_best, spread), log_eta_lo, log_eta_hi)
            max_depth = int(np.clip(np.round(rng.normal(depth_best, max(0.75, 2.2 * (1.0 - progress)))), 3, 12))
            min_child_weight = float(np.clip(10 ** rng.normal(np.log10(2.5), max(0.12, 0.45 * (1.0 - progress))), 0.3, 30.0))
            subsample = float(np.clip(rng.normal(0.88, max(0.025, 0.14 * (1.0 - progress))), 0.55, 1.0))
            colsample_bynode = float(np.clip(rng.normal(0.62, max(0.025, 0.17 * (1.0 - progress))), 0.35, 1.0))
            gamma = float(np.clip(rng.normal(0.08, max(0.02, 0.28 * (1.0 - progress))), 0.0, 2.0))

        eta = float(10 ** log_eta)
        num_round = int(np.clip(np.round(170 - 34 * np.log10(eta / 0.001)), 35, 240))

        # Synthetic objective: valley around eta≈0.018 and depth≈6, with
        # a visible dead-zone penalty for overly deep trees.
        eta_term = 0.014 * ((log_eta - log_eta_best) / 0.55) ** 2
        depth_term = 0.0026 * ((max_depth - depth_best) / 1.65) ** 2
        deep_penalty = 0.010 * max(0.0, max_depth - 8.0) ** 1.25
        subsample_term = 0.0045 * (subsample - 0.88) ** 2
        colsample_term = 0.0035 * (colsample_bynode - 0.62) ** 2
        gamma_term = 0.0015 * gamma
        noise = rng.normal(0.0, 0.00085 if explore else 0.00045)
        value = float(max(0.001, 0.021 + eta_term + depth_term + deep_penalty + subsample_term + colsample_term + gamma_term + noise))

        # Extra metrics are included so --hpo-visualize-metric can be tested.
        log_loss = float(value * 2.4 + 0.015 + rng.normal(0.0, 0.001))
        ece = float(max(0.0005, 0.004 + value * 0.18 + rng.normal(0.0, 0.00035)))
        best_iteration = int(np.clip(num_round * rng.uniform(0.45, 0.92), 10, num_round))
        ...
        trials.append(_DemoHPOTrial(number=i, value=value, values=[value], params=params, user_attrs=user_attrs))

    return _DemoHPOStudy(trials=trials)


def run_hpo_visualization_demo(args) -> List[str]:
    """Generate all HPO visualization assets from synthetic data and exit."""
    header("HPO Visualization Quick Demo")
    os.makedirs(args.logdir, exist_ok=True)
    args.hpo_visualize = True
    study = make_demo_hpo_study(
        n_trials=int(getattr(args, "hpo_demo_trials", 60)),
        seed=int(getattr(args, "hpo_demo_seed", 12345)),
    )
    made = save_hpo_visualizations(study, args, prefix="hpo_demo")
    if made:
        print("[HPO Viz Demo] Wrote demo assets:")
        for path in made:
            print(f"  • {path}")
    else:
        print("[HPO Viz Demo] No assets were written. Check plotting dependencies and selected params/metric.")
    return made
...
def _get_or_build_hpo_dmatrices_tabularized(
    ...
    if len(_HPO_DMATRIX_CACHE) > 8:
        status("HPO cache", f"cache has {len(_HPO_DMATRIX_CACHE)} entries; clearing to avoid memory growth")
    ...
    if cached is not None:
        status("HPO cache", f"HIT horizon_idx={horizon_idx}; reusing cached dtrain/dval")
        return cached  # (dtrain, dval)

    t_build0 = time.time()
    status("HPO cache", f"MISS horizon_idx={horizon_idx}; building dtrain/dval once from tabularized arrays")

    keep_tr = V_tr[:, horizon_idx]
    Xk = X_tr[keep_tr]
    yk = Y_tr[keep_tr, horizon_idx].astype(np.float32)
    if Xk.shape[0] == 0:
        status("HPO cache", f"zero valid TRAIN rows for horizon_idx={horizon_idx}")
        return (None, None)
    ...
    if Xv.shape[0] == 0:
        status("HPO cache", f"zero valid VAL rows for horizon_idx={horizon_idx}")
        return (None, None)

    status("HPO cache", f"valid rows: train={Xk.shape[0]:,}, val={Xv.shape[0]:,}, features={Xk.shape[1]:,}, gpu={is_gpu}, dmatrix={getattr(args, 'xgb_dmatrix', 'dmat')}")
    ...
    status("HPO cache", f"stored cached dtrain/dval for horizon_idx={horizon_idx} in {time.time() - t_build0:.1f}s")
    return dtrain, dval
...
def hpo_multi_objective(
    ...
    if args.train_mode == "tabularize":
        ...
        status("HPO data", f"trial={trial.number} using TABULARIZED HPO data; can_reuse_cached_dmatrix={can_reuse}")
        if can_reuse:
            ...
            status("HPO data", f"trial={trial.number} cache disabled because --hpo-subsample-train={args.hpo_subsample_train}; building fresh sampled DMatrix")
            keep_tr = V_tr[:, horizon_idx]
            Xk = X_tr[keep_tr]
            yk = Y_tr[keep_tr, horizon_idx].astype(np.float32)
...
def hpo_objective(
    ...
    Returns:
        float: selected validation objective metric (lower is better), controlled by --hpo-objective-metric
    """
    t_trial0 = time.time()
    objective_metric_requested = _normalize_hpo_objective_metric(getattr(args, "hpo_objective_metric", "logloss"))
    ...
    status(
        "HPO trial",
        "START "
        f"trial={trial.number} horizon_idx={horizon_idx} "
        f"mode={args.train_mode} objective={objective_metric_requested} "
        f"eta={hpo_params['eta']:.6g} max_depth={hpo_params['max_depth']} "
        f"min_child_weight={hpo_params['min_child_weight']:.6g} "
        f"subsample={hpo_params['subsample']:.3f} "
        f"colsample_bynode={hpo_params['colsample_bynode']:.3f} "
        f"gamma={hpo_params['gamma']:.4g} num_round={hpo_params['num_round']}"
    )
    ...
    if args.train_mode == "tabularize":
        ...
        status("HPO data", f"trial={trial.number} using STREAMING HPO path; DMatrix will be rebuilt from DataIter for this trial")
    ...
    status("HPO trial", f"TRAIN trial={trial.number} num_round={num_round} early_stopping={int(args.hpo_early_stopping)}")
    try:
        bst = xgb.train(
            params=params,
            dtrain=dtrain,
    ...
    # --- Compute validation predictions and objective metrics ---
    y_true = dval.get_label()
    ...
    try:
        ece, mce = compute_calibration_error(y_true_for_metrics, y_pred_probs, n_bins=int(getattr(args, "hpo_ece_bins", 10)))
    ...
    # Select the scalar Optuna objective.
    # Default remains Brier score for backward compatibility, but you can use
    # --hpo-objective-metric logloss to optimize validation log loss, or ece to optimize expected calibration error directly.
    objective_metric = _normalize_hpo_objective_metric(getattr(args, "hpo_objective_metric", "logloss"))
    objective_value = trial_metrics.get(objective_metric, float("nan"))
    try:
        objective_value = float(objective_value)
    except Exception:
        objective_value = float("nan")
    if not np.isfinite(objective_value):
        print(f"[HPO] Trial {trial.number}: objective metric '{objective_metric}' is NaN/inf; returning inf")
        objective_value = float("inf")

    trial_metrics["hpo_objective_metric"] = objective_metric
    trial_metrics["hpo_objective_value"] = float(objective_value)

    # Store as JSON-serializable user attributes
    trial.set_user_attr("metrics", trial_metrics)
    trial.set_user_attr("hpo_objective_metric", objective_metric)
    trial.set_user_attr("hpo_objective_value", float(objective_value))

    status(
        "HPO trial",
        "END "
        f"trial={trial.number} objective={objective_metric} value={objective_value:.8g} "
        f"brier={trial_metrics.get('brier_score', float('nan')):.8g} "
        f"log_loss={trial_metrics.get('log_loss', float('nan')):.8g} "
        f"ece={trial_metrics.get('ece', float('nan')):.8g} "
        f"best_iteration={best_iter} elapsed={time.time() - t_trial0:.1f}s "
        f"cached_dmatrix={used_cache}"
    )
...
def run_hpo_study(
    ...
    status("HPO setup", f"starting Optuna study seed={seed} trials={args.hpo_trials} sampler={args.hpo_sampler}")
    status("HPO setup", f"train_mode_for_HPO={args.train_mode}; objective={getattr(args, 'hpo_objective_metric', 'logloss')}; horizon_strategy={args.hpo_horizon_strategy}")
    if args.train_mode == "tabularize":
        cache_ok = not (getattr(args, 'hpo_subsample_train', None) and args.hpo_subsample_train < 1.0)
        status("HPO setup", f"tabularized arrays will be reused across trials; dmatrix_cache_enabled={cache_ok}")
    else:
        status("HPO setup", "streaming HPO path selected; DataIter/DMatrix will be rebuilt per trial")
    ...
    if args.hpo_sampler.lower() == "nsgaii":
        ...
        hpo_config = {
            'sampler': 'nsgaii',
            'objective_metric': 'multi_objective',
            'n_trials': int(args.hpo_trials),
            'timeout': args.hpo_timeout,
        ...
        if getattr(args, "hpo_visualize", False):
            save_hpo_visualizations(study, args, prefix="hpo_nsgaii")
    ...
    print(f"[HPO] Objective metric: {_normalize_hpo_objective_metric(getattr(args, 'hpo_objective_metric', 'logloss'))}")
    ...
    if args.hpo_horizon_strategy == "first":
        ...
        # Optimize mean selected objective across all horizons
        study = optuna.create_study(
            direction="minimize",
            sampler=sampler,
        ...
        def mean_objective(trial):
            ...
            return float(np.mean(objective_scores))
    ...
    hpo_config = {
        ...
        'objective_metric': _normalize_hpo_objective_metric(getattr(args, 'hpo_objective_metric', 'logloss')),
        'ece_bins': int(getattr(args, 'hpo_ece_bins', 10)),
        'n_trials': int(args.hpo_trials),
        'timeout': args.hpo_timeout,
        'horizon_strategy': args.hpo_horizon_strategy,
        'early_stopping_rounds': int(args.hpo_early_stopping),
    ...
    print(f"[HPO] Saved HPO config to: {hpo_config_path}")
    if getattr(args, "hpo_visualize", False):
        save_hpo_visualizations(study, args, prefix="hpo")
...
def train_xgb_per_horizon_streaming(train_ds, val_ds, args, seed: int):
    ...
    params = make_xgb_params(args, seed=seed)
    status("Final train", f"starting final XGBoost training in STREAMING mode; DataIter will stream chunks into XGBoost once per horizon")
...
def compute_netdry7_features(
    ...
    # NOTE: In the DeepPeat pipeline we may prune ERA5-Land channels (e.g., drop first 4).
    # To stay robust, match multiple likely indices PLUS any explicit precip band name.
    precip_pattern = getattr(args, 'precip_channel_pattern', None) or r"(era5land.*c4|era5land.*c8|era5land.*c12|era5land.*\b(4|8|12)\b|precip|tp|prcp)"
...
def _resolve_seed_model_dir(load_model_dir: str, seed: int) -> str:
    """Resolve a user-provided model directory to the actual directory for this seed."""
    base = os.path.abspath(os.path.expanduser(str(load_model_dir)))
    direct = os.path.join(base, f"xgb_h{int(seed)}.json")
    # direct is not a real pattern check, keep for backwards compatibility if user points at a file dir named oddly
    if os.path.isfile(os.path.join(base, "xgb_h0.json")) or os.path.isfile(os.path.join(base, "xgb_manifest.json")):
        return base
    seed_dir = os.path.join(base, f"seed{int(seed)}")
    if os.path.isfile(os.path.join(seed_dir, "xgb_h0.json")) or os.path.isfile(os.path.join(seed_dir, "xgb_manifest.json")):
        return seed_dir
    raise FileNotFoundError(
        f"Could not find saved XGBoost models under {base!r} or {seed_dir!r}. "
        "Expected files like xgb_h0.json / xgb_manifest.json."
    )


def _load_feature_names_from_dir(model_dir: str) -> Optional[List[str]]:
    ...
    try:
        ...
        print(f"[Load] WARNING: Failed to read feature names from {path}: {e}")
    return None


def _deserialize_saved_calibrator(obj):
    if not isinstance(obj, dict):
        raise ValueError(f"Unsupported calibrator payload type: {type(obj)}")
    typ = str(obj.get("type", "")).lower()
    if typ == "platt":
        return PlattCalibrator(a=float(obj["a"]), b=float(obj["b"]))
    if typ == "isotonic":
        return IsotonicCalibrator(
            x_thresholds=np.asarray(obj["x_thresholds"], dtype=np.float64),
            y_values=np.asarray(obj["y_values"], dtype=np.float64),
        )
    raise ValueError(f"Unsupported calibrator type: {typ!r}")


def load_calibrators_from_dir(model_dir: str):
    """Load saved calibrators from pickle if present, otherwise from JSON.

    Returns:
        tuple[dict, Optional[str], bool]: (calibrators, method, per_regime)
    """
    pkl_path = os.path.join(model_dir, "calibrators.pkl")
    json_path = os.path.join(model_dir, "calibrators.json")
    ...
    if os.path.exists(pkl_path):
        ...
        if os.path.exists(json_path):
            try:
                ...
                method = None
        per_regime = ("global" in calibs) or any(isinstance(k, tuple) for k in calibs.keys())
        return calibs, method, per_regime
    ...
    if "per_horizon" in payload:
        calibs = {int(h): _deserialize_saved_calibrator(c) for h, c in payload["per_horizon"].items()}
        return calibs, method, False

    calibs = {}
    global_fallback = payload.get("global_fallback", {}) or {}
    if global_fallback:
        calibs["global"] = {int(h): _deserialize_saved_calibrator(c) for h, c in global_fallback.items()}
    per_regime_h = payload.get("per_regime_horizon", {}) or {}
    for regime_id, per_h in per_regime_h.items():
        for h, c in (per_h or {}).items():
            calibs[(int(regime_id), int(h))] = _deserialize_saved_calibrator(c)
    return calibs, method, True


def load_xgb_models(model_dir: str, horizons: list[int]) -> tuple[list, dict]:
    """Load one saved XGBoost booster per horizon from a run directory."""
    boosters = []
    for h in horizons:
        path = os.path.join(model_dir, f"xgb_h{int(h)}.json")
        if not os.path.exists(path):
    ...
    manifest = {}
    mpath = os.path.join(model_dir, "xgb_manifest.json")
    if os.path.exists(mpath):
        try:
            with open(mpath, "r") as f:
                manifest = json.load(f)
            print(f"[Load] Loaded {mpath}")
        except Exception as e:
            print(f"[Load] WARNING: Failed to read manifest {mpath}: {e}")
            manifest = {}
...
def parse_args():
    ...
    p.add_argument("--hpo-objective-metric", choices=["brier_score", "brier", "log_loss", "logloss", "ece", "mce"], default="logloss",
                   help="Single-objective HPO metric to minimize. Use 'ece' to optimize expected calibration error directly. Default: brier_score.")
    p.add_argument("--hpo-ece-bins", type=int, default=10,
                   help="Number of probability bins used when computing ECE/MCE during HPO. Default: 10.")
    ...
    p.add_argument("--hpo-visualize", action="store_true",
                   help="After HPO, save 3D landscape, contour, parallel-coordinate, and optional Plotly HTML visualizations.")
    p.add_argument("--hpo-visualize-params", type=str, default="eta,max_depth",
                   help="One pair or multiple semicolon-separated HPO param pairs for landscape/contour plots, e.g. 'eta,max_depth' or 'eta,max_depth;eta,min_child_weight'.")
    p.add_argument("--hpo-visualize-metric", type=str, default="value",
                   help="Metric to plot on the landscape z-axis/color. 'value' means the selected --hpo-objective-metric; use 'brier_score', 'log_loss', 'ece', etc. for explicit metrics.")
    p.add_argument("--hpo-visualize-color-by", choices=["trial", "trial_number", "value"], default="trial",
                   help="Color parallel-coordinate lines by trial sequence or objective value. 'trial_number' is accepted as an alias for 'trial'.")
    p.add_argument("--hpo-landscape-grid-size", type=int, default=160,
                   help="Interpolation grid size for smoother HPO 3D landscape and contour plots. 160-300 is usually enough; very large values increase rendering time/memory.")
    p.add_argument("--hpo-visualize-demo", action="store_true",
                   help="Generate a fast synthetic example of the HPO plots and exit; no dataset, XGBoost training, or Optuna study required.")
    p.add_argument("--hpo-demo-trials", type=int, default=60,
                   help="Number of synthetic trials to draw for --hpo-visualize-demo. Default: 60")
    p.add_argument("--hpo-demo-seed", type=int, default=12345,
                   help="Random seed for --hpo-visualize-demo. Default: 12345")
    p.add_argument("--hpo-parallel-params", type=str,
                   default="eta,max_depth,min_child_weight,subsample,colsample_bynode,gamma,num_round",
                   help="Comma-separated params to include in the HPO parallel-coordinate plot.")
    ...
    p.add_argument("--plot-dpi", type=int, default=850)
    ...
    p.add_argument("--load-model-dir", default=None,
                   help="If set, skip XGBoost training and load saved per-horizon models from this directory. The directory may directly contain xgb_h*.json or contain seed subdirectories like seed42/.")
    p.add_argument("--no-load-calibrators", action="store_true",
                   help="When using --load-model-dir, do not auto-load saved calibrators.json/.pkl from that directory.")
...
def run_one_seed(base_args, seed: int, train_ds, val_ds, test_ds, calib_ds):
    ...
    print(f"  • load_model_dir............. {args.load_model_dir}")
    ...
    if getattr(args, "load_model_dir", None):
        loaded_model_dir = _resolve_seed_model_dir(args.load_model_dir, args.seed)
        print(f"[Load] Skipping training and loading saved boosters from: {loaded_model_dir}")
        boosters, loaded_manifest = load_xgb_models(loaded_model_dir, [int(h) for h in args.horizons])
        params = dict(loaded_manifest.get("params", {}) or {})

        loaded_feature_names = _load_feature_names_from_dir(loaded_model_dir)
        if loaded_feature_names is not None:
            feature_names = loaded_feature_names
            print(f"[Load] Loaded {len(feature_names)} feature names from {loaded_model_dir}")

        manifest_n_features = loaded_manifest.get("n_features")
        if manifest_n_features is not None and int(manifest_n_features) != int(n_features):
            raise RuntimeError(
                f"Loaded model expects n_features={int(manifest_n_features)}, but rebuilt dataset exposes {int(n_features)} features."
            )

        manifest_calib_method = str(loaded_manifest.get("calibration_method", "none") or "none")
        if getattr(args, "calibration_method", "none") == "none" and manifest_calib_method != "none":
            args.calibration_method = manifest_calib_method
            print(f"[Load] Using calibration_method from manifest: {args.calibration_method}")
    ...
    if boosters is None:
        ...
        if args.train_mode == "tabularize":
            ...
            status("Data", f"tabularized train arrays X={_shape_str(X_tr)} Y={_shape_str(Y_tr)} V={_shape_str(V_tr)}")
            if X_va is not None:
                status("Data", f"tabularized val arrays X={_shape_str(X_va)} Y={_shape_str(Y_va)} V={_shape_str(V_va)}")
            ...
            if args.hpo:
                ...
                if args.hpo_params_in:
                    ...
                    print(f"[HPO] Optimizing: {_hpo_metric_display_label(getattr(args, 'hpo_objective_metric', 'logloss'), args)}")
                    ...
                    hpo_save_data = {
                        **best_hpo_params,
                        "study_info": {
                            ...
                            "best_objective_metric": getattr(args, "hpo_objective_metric", "logloss"),
                    ...
                    print(f"\n[HPO] ====== Best Trial ======")
                    print(f"[HPO] Best objective ({getattr(args, 'hpo_objective_metric', 'logloss')}): {study.best_value:.6f}")
            ...
            status("Final train", f"starting final XGBoost training in TABULARIZED mode with best/current params; num_round={args.xgb_num_round}")
    ...
    loaded_calibrators = False

    if loaded_model_dir is not None and not bool(getattr(args, "no_load_calibrators", False)):
        try:
            calibrators, loaded_calib_method, per_regime_calib = load_calibrators_from_dir(loaded_model_dir)
            if calibrators:
                loaded_calibrators = True
                if loaded_calib_method and getattr(args, "calibration_method", "none") == "none":
                    args.calibration_method = str(loaded_calib_method)
                print(f"[Load] Loaded calibrators from {loaded_model_dir} (method={args.calibration_method})")
        except Exception as e:
            print(f"[Load] WARNING: Failed to load calibrators from {loaded_model_dir}: {e}")

    if (not loaded_calibrators) and args.calibration_method != "none":
        if calib_ds is None:
...
def main():
    ...
    if getattr(args, "hpo_visualize_demo", False):
        run_hpo_visualization_demo(args)
        return

    if not _HAVE_DATASET_BUILDER:
        print("[FATAL] Training/evaluation requires joint_peat_dataset_builder_cluster.py to be importable.")
        raise SystemExit(2)
    ...
    if args.load_model_dir is None and args.train_mode == "stream" and not _have_xgb_dataiter():
